...
pub async fn shard_storage(State(state): State<AppState>) -> Json<serde_json::Value> {
    ...
    Json(serde_json::json!({
        ...
        "auto_manage_enabled": state.shared_state.cfg().auto_manage.enabled,
        "auto_manage_max_storage_mb": state.shared_state.cfg().auto_manage.max_storage_mb,
        "max_disk_mb": state.shared_state.cfg().resources.max_disk_mb,
...
pub async fn model_metadata(
    ...
    // `general.file_type` is llama.cpp's `llama_ftype`, and the numbering is
    // NOT in quality order — the K-quants were appended as they were invented,
    // so 10..18 run Q2_K, Q3_K_S/M/L, Q4_K_S/M, Q5_K_S/M, Q6_K. This table was
    // out of step from 8 onward: it read 15 as Q2_K, so every Q4_K_M model —
    // which is most of them — reported itself as the worst K-quant there is,
    // and a Q6_K read as Q4_K_M. Values from `llama_ftype` in llama.cpp's
    // `include/llama.h`; 4..6 and 33..35 are retired and deliberately absent.
