...
/// Chunk size for parallel downloads (8 MB — small enough for good parallelism,
/// large enough to avoid excessive per-chunk overhead).
const PARALLEL_CHUNK_SIZE: u64 = 8 * 1024 * 1024;

/// Maximum concurrent chunk downloads per shard.
const MAX_PARALLEL_CHUNKS: usize = 4;

/// Tracks chunks assigned to different peers for parallel shard download.
#[derive(Debug)]
pub struct ParallelChunkTracker {
    /// Shard being downloaded.
    pub shard_id: ShardId,
    /// Total shard size in bytes.
    pub total_size: u64,
    /// Chunks: (offset, size, assigned_peer, status).
...
pub struct ChunkState {
    ...
    pub size: u64,
    /// Peer assigned to download this chunk (None = unassigned).
    pub assigned_peer: Option<NodeId>,
    /// Download status.
    pub status: ChunkStatus,
}

/// Download status for a single chunk.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ChunkStatus {
    /// Not yet started.
    Pending,
    /// Requested from a peer, waiting for data.
    InFlight,
    /// Data received and written to disk.
    Complete,
    /// Download failed (peer disconnected, timeout, etc.).
    Failed,
}

impl ParallelChunkTracker {
    /// Create a tracker for a shard, splitting it into chunks.
    pub fn new(shard_id: ShardId, total_size: u64) -> Self {
        let mut chunks = Vec::new();
        let mut offset = 0u64;
        while offset < total_size {
            let size = (total_size - offset).min(PARALLEL_CHUNK_SIZE);
    ...
    /// Assign pending chunks to available peers (up to MAX_PARALLEL_CHUNKS in-flight).
    pub fn assign_chunks(&mut self, available_peers: &[NodeId]) -> Vec<(usize, NodeId, u64, u64)> {
        let in_flight = self
            .chunks
            .iter()
            .filter(|c| c.status == ChunkStatus::InFlight)
            .count();
        let slots = MAX_PARALLEL_CHUNKS.saturating_sub(in_flight);
        if slots == 0 || available_peers.is_empty() {
            return vec![];
        }
        ...
        for (chunk_idx, chunk) in self.chunks.iter_mut().enumerate() {
            if assignments.len() >= slots {
                break;
            }
            if chunk.status == ChunkStatus::Pending {
                let peer = available_peers[peer_idx % available_peers.len()].clone();
                chunk.assigned_peer = Some(peer.clone());
                chunk.status = ChunkStatus::InFlight;
                assignments.push((chunk_idx, peer, chunk.offset, chunk.size));
                peer_idx += 1;
            }
        }
        assignments
    ...
    pub fn complete_chunk(&mut self, offset: u64) {
        if let Some(chunk) = self.chunks.iter_mut().find(|c| c.offset == offset) {
            chunk.status = ChunkStatus::Complete;
        }
    }

    /// Mark a chunk as failed (will be reassigned on next assign_chunks call).
    pub fn fail_chunk(&mut self, offset: u64) {
        if let Some(chunk) = self.chunks.iter_mut().find(|c| c.offset == offset) {
