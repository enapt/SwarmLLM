...
- `state.metrics` (`MetricsProviders`) — `node_stats`, `inference_requests_total`, `channel_metrics`, `providers_config`, `swarm_capacity` (R110), etc.
...
- 957 lib tests passing + 8 ignored (env-var-gated real-model + manual smoke), 75 integration tests in `tests/integration/`, 1 ignored end-to-end (`cargo test --test integration_phase10_11 -- --ignored`), clippy clean
...
All 20 build phases complete. All subsystems wired — no stubs. 943 lib tests + 75 integration tests passing; 8 lib + 1 e2e ignored (env-var or manual). Latest: R133 — Quant choice automation (FUTURE_WORK closure): expanded `Quantization` enum to 28 variants spanning K-quants (Q2_K..Q6_K), legacy (Q4_0..Q5_1, Q8_0), I-quants (IQ1_S..IQ4_NL), and floats (F16/BF16/F32) plus `Unknown` fallback. Each variant exposes `parse(str)` / `bits_per_weight()` / `quality_score(0..100)` / `label()` — quality scores calibrated against llama.cpp perplexity-loss tables. New `model/auto_manage/quant.rs` module computes per-family quant recommendations: groups registry models by inferred base name (model name with quant tag stripped), picks the highest-quality variant whose VRAM footprint fits the swarm budget (local GPU OR aggregate-pool VRAM divided by 3-replica target). Rationale carries human-readable i18n tag (`quant.rec.{best_fit,would_upgrade,too_big,cpu_only,no_variants}`) translated across all 21 locales. New `GET /api/admin/quant-recommendations` endpoint + `state.models.quant_recommendations` ArcSwap cache refreshed alongside `wishlist` on every auto-manage tick + WS stats build. Read-only — doesn't auto-switch which quant the daemon hosts; surfaces a hint for the user. 7 unit tests including a registry-backed end-to-end. R132 — MoE per-arch routing config (FUTURE_WORK closure): new `MoeGatingFunc { Softmax, Sigmoid }` + `MoeRoutingConfig { gating_func, renormalize_weights }` in `src/inference/layers/mod.rs`. `topk_cpu` extended to branch on the config (fast softmax+renorm path preserved); `MoeFfn` carries the config and passes it on forward. GGUF loader reads `{arch}.expert_gating_func` (uint, 1=softmax / 2=sigmoid; matches llama.cpp's enum) + `{arch}.expert_weights_norm` (bool) once and threads it into all 3 MoeFfn construction sites. Default (Softmax + renorm) matches Mixtral / Qwen3-MoE / Llama 4 — historical behaviour preserved. Differentiates DeepSeek-V2 strict (softmax + no renorm) and DeepSeek-V3 sigmoid gating. 3 new unit tests covering each non-default mode. R131 — pool-state gossip debounce (FUTURE_WORK closure): `PoolManager` gains `last_pool_gossip_at: Option<Instant>` + `pool_gossip_dirty: bool` rate-limiter (15s min interval). Post-acceptance broadcasts now go through `maybe_gossip_pool_state` instead of forcing an immediate `gossip_pool_state` — 50 acceptances in <15s coalesce to 1 immediate + 1 trailing instead of 50. New 3s pool-coalesce timer arm in the select! loop drains the dirty flag once the cooldown elapses; periodic full-broadcast tick still bypasses the gate (anti-poison + late-joiner recovery). 3 unit tests cover first-call fires / second-within-window debounces / post-cooldown drains. Wire-format unchanged — deferred true diff variants until WAN bench shows actual size constraint. R130 — cross-pool wishlist gossip (FUTURE_WORK closure): new `SwarmMessage::WishlistAnnouncement` variant on `swarm/regions` topic carries publisher top-K (model_id, 0..100 score) entries; receivers feed `state.models.foreign_wishlist: DashMap<(NodeId, ModelId), (score, ts_ms)>` (capped 10K entries, 2h freshness window); `compute_wishlist` adds a 0..10 boost blending publisher count (log10 breadth) × max score (depth) + new `wishlist.why.other_nodes_want_this` i18n tag across 21 locales. Publishing opt-in via `auto_manage.wishlist_gossip_publish` (default off — privacy default); ingest always on, so privacy-conscious nodes still benefit. R127 — 4 more FUTURE_WORK closures: orphaned `models.hf_score_breakdown` (+3 siblings) removed across 21 locales (swarm-tab already shows human verdict via `_fitPill()`), `dashboard.api_log_link` translated across 21 locales, country names via `Intl.DisplayNames` (replaces hand map), and `ModelRegistry.global_holder_count` + DHT plumbing so prune's `redundancy_ratio` is no longer capped at 50 holders. R126 — bulk follow-ups closure (10 items from `docs/FUTURE_WORK.md`): R121 Setup-wizard contribution toggle, R125 README `<details>` wrap, `worker_*` event removal, "GGUF Metadata" → "Technical Details" rename + Advanced collapse, honest end-to-end encryption copy across 21 locales, manifest tensor-cap ActivityEvent, `LayerResult` wire-format trailers (matched_stop_sequence + token_logprobs, 0x04/0x05 markers), cross-node logprobs through distributed pipeline, `SwarmMessage::CancelInference` cross-wire cancel for remote-generate, TrustManager in-memory cache + startup hydrate. Deferred items in `docs/ARCHITECTURE.md` § "Deferred Items".
