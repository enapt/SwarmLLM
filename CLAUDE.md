...
├── frontend/      (index.html + 11 HTML templates, css/, js/{core/4,components/19,init.js,i18n.js,providers.js,neural-bg.js,topojson-client.min.js}, i18n/)
...
- 12 HTML modals/templates incl. R127 `#welcome-modal` (first-run tour). 10 `<template>` elements for repeating UI structures (session items, chat messages, toasts, model cards, etc.)
...
All 20 build phases complete. All subsystems wired — no stubs. **1174 lib tests + 75 integration + 2 repo-consistency tests passing**; 8 lib + 1 e2e ignored (env-var or manual). Clippy clean default + features dev,claude-subscription + `--features llama`.
...
**GPU-verified on an RTX 3070** (maintainer-requested CUDA build): `gpu_layers`
0→CPU / -1→CUDA confirmed by VRAM deltas; a forced CUDA OOM killed the worker
and reclaimed **7951→120 MiB in 6s**, with the retry succeeding on a CPU-pinned
worker; live `gpu_vram_used_mb` matched `nvidia-smi` exactly. The cancellation
test found a **real gap** — `spawn_split_stream` discarded its JoinHandle, so
nothing dropped the generate future and a hung-up client left the worker at 754%
CPU. Fixed by `select!`ing on `Sender::closed()`; re-verified 780%→0.0%.
