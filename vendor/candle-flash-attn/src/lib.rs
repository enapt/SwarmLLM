...
// SwarmLLM patch: `bf16` dropped with the bf16 kernels (see build.rs).
use half::f16;
...
impl candle::CustomOp3 for FlashAttn {
    ...
    fn cuda_fwd(
        ...
        match q.dtype() {
            ...
            // SwarmLLM patch: bf16 dispatch removed along with the 18
            // `_bf16_` kernels (see build.rs). `run_attention` casts to F16
            // before every call, so this arm had no reachable caller; leaving
            // it would reference kernels that are no longer compiled in.
            candle::DType::BF16 => candle::bail!(
                "flash-attn: this build compiles only the f16 kernels — cast to f16 first"
            ),
