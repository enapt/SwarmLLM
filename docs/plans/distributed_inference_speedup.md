...
- **Item 8 — Cross-node prefix cache sharing** 🟡 PHASE 1 LANDED 2026-04-19. Announce BLAKE3 prompt-prefix hashes over gossip; peers that already prefilled a shared prefix serve KV blocks on demand. Content-addressed KV shards — fits our existing shard announcement infrastructure. Potentially novel for P2P.
  ...
  - **Phase 2b (this commit, 2026-04-19).** Worker↔daemon IPC cascade + admit-time hook + serving-side worker extraction.
    - New IPC verbs: `WorkerMsg::PrefixFetchProbe { request_id, model_id, blocks }` (worker asks daemon to resolve a block-hash manifest) + `DaemonMsg::PrefixFetchResult { request_id, matched_tokens, payload }` (daemon responds with snapshot bytes or `None`) + `DaemonMsg::ExportPrefixSnapshot { request_id, model_id, block_hash }` (daemon asks worker to serialize a cached snapshot) + `WorkerMsg::PrefixSnapshotResponse { request_id, payload }` (worker replies with serialized bytes or None).
    - Worker main loop: `pending_fetches: PrefixFetchWaiterMap` registers per-probe oneshots; the reader task short-circuits matching `PrefixFetchResult` arrivals and fulfils them inline, bypassing the main message queue so `handle_generate` can await without deadlocking.
    - `try_remote_prefix_hydrate` helper: builds the chained-hash manifest for the prompt, installs an RAII-cleaned oneshot, sends a `PrefixFetchProbe`, awaits with `PREFIX_FETCH_TIMEOUT_MS = 500ms`, then calls `PrefixCache::hydrate_request_from_bytes` on hit. Returns `0` for any failure path (caller falls through to normal prefill).
    - Wired into `handle_generate` AND `try_register_generate_slot` (which becomes async). Both now probe cross-node only when the LOCAL `prefix_cache.lookup` misses — avoiding a pointless round trip when the in-process cache would serve.
    - Serving side: `ModelProcessPool::fetch_local_snapshot` sends `ExportPrefixSnapshot` to the worker via a fresh request_id registered in the standard response map, awaits `PrefixSnapshotResponse` (500ms timeout). Worker's `export_snapshot_for_hash` iterates loaded models' `kv_model_key`s and calls `PrefixCache::export_snapshot_bytes` (new helper that finds the matching entry, narrows to the requested block boundary, serializes).
    - NetworkManager: inbound `SwarmRequest::PrefixKvFetch` now stashes the `ResponseChannel` in a new `pending_prefix_kv_inbound` map (cap 256) keyed by a fresh ticket Uuid, spawns a task that calls `fetch_local_snapshot`, and emits `NetworkCommand::DeliverPrefixKvResponse { ticket, ... }` which pops the stored channel and sends the reply. New `internal_cmd_tx/rx` channel lets manager-spawned tasks push commands back into the event loop. Unauthenticated peers and queue-full situations reply `None` immediately.
    - `ModelProcessPool` additions: `prefix_probe_tx` OnceLock (analogous to `prefix_manifest_tx`), `set_prefix_probe_tx`, `send_prefix_fetch_result`, `fetch_local_snapshot`, `get_existing` helper.
    - New daemon background task `spawn_prefix_probe_handler` drains probes, walks the local cross-node index to pick the best peer, dispatches `NetworkCommand::SendPrefixKvFetch`, awaits the peer's reply (400ms timeout), and routes the result back to the worker via `send_prefix_fetch_result`.
    - Tests: +2 (export-snapshot roundtrip, export-snapshot miss-on-unknown-hash). **765 lib + integration** on `dev,claude-subscription` (up from 763). Clippy clean.
    - NOT yet in this phase: end-to-end 2-node integration test (needs real multi-node harness — a single-process mock would duplicate large chunks of the network + IPC stack). Deferred to Phase 4 bench.
  - **Phase 3 (this commit, 2026-04-20).** Trust-gated fetching + snapshot integrity checks + peer trust penalties.
    - New config `InferenceConfig::cross_node_prefix_trust_min: f32` (default `0.5`, same as `DEFAULT_TRUST`). Peers below the threshold are skipped at probe time — no wire round trip, so a misbehaving peer can't waste our time after it's been flagged.
    - Daemon-side `verify_fetched_snapshot` runs AFTER the fetch returns bytes, BEFORE they're handed to the worker for hydration. Three checks: (1) deserialize succeeds; (2) BLAKE3 chain over declared tokens matches the requested block hash at the sender's block size; (3) every K/V tensor contains only finite values (no NaN/Inf). On any failure: bytes are dropped, caller gets `None`, and the sender's trust score is penalized via `TrustEvent::SpotCheckFail` (-0.1).
    - Wire format: `SnapshotHeader` gained an optional `block_size: Option<usize>` field (serde-defaulted for backward compatibility). Phase-3 senders record their configured block size; receivers use it to verify the hash chain deterministically. Pre-Phase-3 snapshots fall back to trying the common defaults (32, 64, 128) — covers the three active defaults in the swarm without growing state.
