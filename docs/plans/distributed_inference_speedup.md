...
>
> **Item 8 — Phases 1, 2a, 2b LANDED 2026-04-19. Next: Phase 3 (trust
> integration + re-verification) or Phase 4 (bench) based on priority.**
>
...
- **Item 8 — Cross-node prefix cache sharing** 🟡 PHASE 1 LANDED 2026-04-19. Announce BLAKE3 prompt-prefix hashes over gossip; peers that already prefilled a shared prefix serve KV blocks on demand. Content-addressed KV shards — fits our existing shard announcement infrastructure. Potentially novel for P2P.
  ...
  - **Phase 3 (next).** Trust-gated re-verification. Untrusted peers' KV blocks get re-verified by running a single-token forward at `matched_tokens` and comparing the logit argmax against what our model would produce at that position. High-trust peers skip re-verification. Wire into `state.credits.trust_manager`.
