...
│   ├── Decoded-so-far ids (2026-09-21) — `LayerForward` trailer `0x08`, AAD-bound, carrying the tokens generated so far to the segment that SAMPLES so it can apply `frequency_penalty` / `presence_penalty`. **The field existed from the start and never reached the wire**: both binary encoders omitted it and both decoders set it empty, so a distributed request with penalties set had them silently dropped whenever the last segment was remote. Emitted only when non-empty and only to a peer advertising `features::FORWARD_GENERATED_IDS` — an older peer rebuilds the seal's AAD from the trailers it parsed, so an unrecognised one breaks every encrypted forward to it.
│   ├── Session key confirmation (2026-09-21) — `SwarmMessage::SessionKeyConfirm{session_id, node_id, sealed}`, the third leg of an ephemeral rekey. The responder of an exchange parks the key it derives and keeps sealing with the one both ends agree on until this (or any message under the new key) opens; the initiator seals `SESSION_CONFIRM_MARKER` the instant it installs. Without it a lost exchange reply left the responder holding a key the initiator never saw — one direction dead, neither end able to notice. WireGuard's rule; gated on `features::SESSION_KEY_CONFIRM` because a peer that cannot confirm must be answered the old way. → `docs/invariants/network.md`
...
- `GET    /api/admin/models/{id}/pipeline-plan` — Pipeline assembly plan: ordered segments + holder candidates per shard window. **A preview, so "there is no route right now" is an ANSWER, not a failure**: it returns 200 with `routable: false` and a `reason` carrying the same `message` / `hint` / `hint_key` a real request would have been given, and the dashboard renders that where the route line would go. Only a model id this node has never heard of is still an error (404). It used to report every routing outcome as a failed request, which both hid the reason from the user and filled the node's log with `API request failed status=503` lines that no user request produced
...
- **Swarm background**: animated canvas flocking network behind the dashboard (`frontend/js/neural-bg.js`). Up to 90 boids (scaled by viewport area) with connecting links, feeler tendrils, gentle drift and mouse repulsion/glow. State-reactive colouring: blue (idle) → cyan (active inference) → red-orange (unhealthy/disconnected); peer count boosts vibrancy and active requests trigger firing pulses.
  **It is the one thing on the page with an ongoing power cost, so it is bounded at four levels.** It advances at 30 Hz rather than the browser's 60 while the pointer is in play; drops to 10 Hz once the pointer has been still for 4 s, which is the ordinary state of a dashboard left open; drops to 8 Hz whenever `state.active > 0` (this page is usually open on the machine running the node, so the decoration yields to the real work — the slowest applicable rate wins); and stops entirely when the tab is hidden. Users can switch it off in Settings (`App.SWARM_ANIM_KEY`), and it defaults to OFF where the device reports `prefers-reduced-motion: reduce` — a canvas loop is invisible to the CSS media query that honours that everywhere else on the page. Turning it off cancels the frame loop rather than hiding the canvas, since a hidden animation costs what a visible one does.
  Measured 2026-09-17 (min-of-5, physics and geometry only, rasterisation on top): one frame costs 0.97 ms at 1280x800, so **2.9% of one core with the pointer moving, 1.0% in the ordinary calm state and 0.8% while serving** — against **10.4%** before this work, i.e. roughly a tenth of what it was in the common case. Heap is flat over 20 000 frames (plateaus at 0.21 MB and stops), because the neighbour scratch, the link buckets and the link list are all reused rather than reallocated per frame. Harness: `<scratchpad>/bench_swarm_anim.js` — the cost is dominated by boid DENSITY rather than count, so a small dense window costs more than a large sparse one.
...
- **Binary signature on auto-update (audit_2026-04-29 C1)** — **CLOSED 2026-09-19.** `src/update_signature.rs` verifies a detached minisign signature before an update is staged or applied, against a public key compiled in from `release_pubkey.txt` whose secret half exists only on the maintainer's machine and never in CI. What is signed is each asset's `.sha256` sidecar rather than the binary — the updater already proves the bytes match the sidecar, so authenticity carries in two hops, and signing ~700 bytes per release instead of ~1.5 GB is what lets the key stay offline at all (the cost of signing binaries is exactly what pushes a signing key into CI, where a compromised workflow can use it). The asset name and version are bound into minisign's signed trusted comment and CHECKED, not displayed, so a genuine signature for one asset cannot be replayed as another's. Refusal is fail-closed at both the download and the apply path, and CI can no longer publish — it leaves the release a draft for the offline signing step. ⚠ **This does NOT defend against a build pipeline compromised before it produced the binary** — signing a hash cannot establish that the hash is of something good; that needs reproducible builds and remains open. Procedure, custody and rotation are documented under docs/, in the release-signing guide. Guards: `an_asset_the_updater_can_ask_for_is_an_asset_we_sign`, `the_release_workflow_refuses_to_publish_without_signatures`.
