...
- **Feature-gated + prefer-direct.** `NodeCapability` advertises
  ...
  FORWARD_GENERATED_IDS, FORWARD_PRE_EMBEDDED, FORWARD_REFUSAL_REASON}`); a node only attempts a relayed send when
