...
| **MessageDispatcher** | `src/daemon/dispatch/mod.rs` | Routes inbound network messages to appropriate subsystems. The only consumer of `network_out`, so a stall here takes the node out of the swarm silently — it writes a liveness marker that HealthMonitor watches from another task, because the supervisor only sees a task that exits or panics, never one that is merely stuck |
...
| **AutoShardManager** | `src/model/auto_manage/` | VRAM-aware shard acquisition + smart pruning (manager, scoring, download, prune, scan, vram, wishlist, quant). R111: refreshes the user-visible wishlist at the end of every tick. R141: wishlist now includes a `Candidate` status that merges HF trending entries the swarm hasn't adopted (cap 24) for one-click user adoption. |
| **HfWatcher** (R112) | `src/model/huggingface/watcher.rs` | Background task polling HuggingFace's trending GGUF feed once per hour. Caches the snapshot on `state.models.hf_trending_cache` (consumed by the wishlist scorer) and auto-promotes models from `Discovered` to `DemandVerified` when they cross the per-publisher download floor + 24h age. **R141** introduced a two-tier threshold via `TRUSTED_HF_PUBLISHERS` allowlist: curators (meta-llama, Qwen, mistralai, bartowski, unsloth, …) promote at 10k downloads; unknown publishers retain the 100k floor. The 24h age gate applies to both tiers. NonCritical — HF outages don't escalate to a daemon crash. Opt-out via `auto_manage.hf_watcher_enabled = false`. |
| **UpdateChecker** | `src/update.rs` | Periodic GitHub release polling, SHA256-verified binary download, atomic apply. **Skipped entirely** when `auto_update = "disabled"` (default until binary signing C1 lands), so the supervisor doesn't log a misleading "exited unexpectedly" warning. Picks the release asset matching both the platform **and the build variant** (`""` / `-cuda` / `-gpu`, from `update_asset_name()`) — a GPU build never updates itself to the same-platform CPU binary. When no asset matches the running variant it offers no update rather than installing a different one. |
