...
| 85 | **A distributed pipeline never checks the prompt against the model's context window** | **OBSERVED 2026-09-16 while fixing report #001; NOT reproduced as a failure, and deliberately not fixed in that commit.** The window check lives in `model_worker.rs::resolve_max_new_tokens`, called from the two tokenization sites in the worker — so it covers the LOCAL path and the `remote_generate` fast path (the peer running the whole request is a worker too, and `SamplingParams.max_tokens_explicit` now rides the wire so that peer resolves it correctly). It does NOT cover the coordinator-driven pipeline: `pipeline/distributed.rs` reads `self.request.sampling_params.max_tokens` at line 28 and loops `for seq_num in 0..max_tokens` with no reference to any context window, and the segment holders only perform layer forwards, so nothing on that path asks the question. **This is not a regression from #001's fix** — the check never existed there, and the flat 2048 fallback still goes on the wire unchanged, so behaviour on that path is byte-identical before and after. What is unknown is what actually happens when `prompt + max_tokens` exceeds the window on a distributed run: the KV grows past the declared window on each holder, and whether that errors, silently truncates or degrades has not been established. ⚠ **Establish that before designing anything** — the obvious 'just call the same helper' does not fit, because the coordinator cannot know the effective window (it is a LOAD-time property: GGUF `context_length` after `effective_context_length`, `MAX_SEQ_LEN_OVERRIDE` and the memory budget have each had a say, and only the holder knows the result). | UNKNOWN severity, needs a reproduction first. Noticed because a test request for an explicit over-large `max_tokens` on a 2048-window model was SERVED rather than refused, and the route headers said why: `x-swarm-route: distributed`, `x-swarm-peers: 1`. Reading the response alone would have looked like the fix failing; the headers are what identified it as a different path (the #552/#555 lesson — establish which PATH answered). Low priority: a small-context model is also the one least likely to be split across nodes |
