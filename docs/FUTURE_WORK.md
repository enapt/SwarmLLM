...
| 84 | **The activity feed reported the gossip timer, not the swarm — and the 100-entry ring meant nothing else survived a minute** | FIXED 2026-09-14, found while measuring something else. Peers re-announce the parts they hold on a timer, and every announcement emitted a `model/shard_announced` ActivityEvent. **Measured on the deployed .181 binary over sixty seconds: 112 events, 103 of them this one** (1.9/s). `emit_activity` caps `activity_history` at **100**, so the ring turned over in under a minute — and that ring is not just the Activity panel: it is the **replay a dashboard receives when it opens**, and the **"recent activity" section of the pasteable diagnostics report**. Both were therefore ~100% peer announcements containing nothing the user had done, which is worth noting is exactly what the live node's own diagnostics showed when pulled cold. Nothing raised a toast, so this was invisible as noise and only visible as absence. **The lesson was already in the file, one function below the defect**: `note_build_tag` logs "ONCE per transition rather than once per announcement… a peer repeats itself indefinitely" — written for the identical peers and the identical timer, and not carried to the user-facing surface. `record_shard_holder_with_build` now returns whether the claim was a state CHANGE (a holder not previously recorded for that shard) and the feed emits only on one. Learning a build tag for a known holder is deliberately not a change, or the feed would fire again for every peer that upgraded; a holder returning after a retraction deliberately is. The per-announce `DIAG: shard announce ingested` is untouched — its own comment says it exists precisely because the ring cannot be the durable record. Test: `a_re_announcement_of_a_known_holder_reports_no_change` |
