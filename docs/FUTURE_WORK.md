...
| 48 | Nothing rebuilds the cloud routing catalog except someone opening the admin page | **HALF FIXED 2026-09-11** (field-reported the same day). `provider_model_map` is what decides whether a cloud model is proxied or refused, and its ONLY writer is the `GET /api/admin/provider-models` handler — no startup fill, no timer. The destructive half is fixed: that handler used to `.clear()` the whole map and repopulate from whatever came back, and a fetch that TIMED OUT was indistinguishable from a provider with no models, so one slow provider erased its own entries and a bad minute erased every provider's. `merge_provider_catalog` now replaces per provider and leaves a non-answering one alone. **Still open: nothing populates it without a human.** A node whose admin page is never opened routes only the cloud models whose id resolves by prefix; the rest answer 404 in ~1ms, from us, with the provider never asked. Field evidence: `cloud_models_available` read 0 for days while the dashboard still listed 80 models — the display cache keeps its previous answer on a failed refresh and the routing map did not, so the two can disagree completely |
