...
| 46 | A tool schema reaches the model with its keys alphabetised | **Found 2026-09-11 while making `tojson` match `transformers`.** Our render is otherwise byte-identical to HuggingFace's, and the GLM-4 parity test pins that; the one difference is key ORDER. A tool definition is a `serde_json::Value` by the time it reaches the renderer, and `serde_json` is built here without `preserve_order`, so its map is a `BTreeMap` and `{"type", "function"}` comes out `{"function", "type"}`, `{"name", "description", "parameters"}` comes out `{"description", "name", "parameters"}`. Every tool-rendering template is affected — Qwen3 and Llama-3.1 as much as GLM-4 — and the model was trained and validated against the author's order. **The fix is one Cargo feature (`serde_json/preserve_order`) with a WIDE blast radius**: every map in the daemon, including API responses and redb values, would switch from alphabetical to insertion order. Not obviously wrong, but it is a serialization-wide change to buy a schema-ordering nicety, and there is no evidence yet that a model cares. Measure first: render one model's tools both ways and compare tool-call rates. ⚠ **There is now direct evidence that schema formatting is not cosmetic to a small model.** Changing only the SEPARATORS — `,`/`:` to Python's `, `/`: ` — moved Qwen2.5-Coder-7B at `temperature: 0` from wrapping its call in `<tools>` to wrapping it in `<xml>`, verified in both directions against a baseline binary. Whitespace did that; key order is a larger perturbation than whitespace |
