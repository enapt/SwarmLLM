...
| 65 | The UI says both "shard" and "part" for the same thing, and both "peer" and "computer" | **DONE 2026-09-14, both halves.** The machine word went to "computer" across all 21 locales (117 strings each), with the pool feature keeping "device" by the user's decision — machines you own and link are a different idea from anyone's computer on the swarm, and "My Devices" is a nav destination named for the first. A **fifth** surface appeared beyond the four #609 names: `ShardId::display_index` RETURNED the word (`"shard 15"`), a shared helper in the types crate, nowhere near any message — which also explained a never-reported defect visible on screen the whole time, "P2P: downloading shard shard 15 from peer", where two callers wrote the noun the helper already supplied (gotcha #610). Translations were mapped, not retyped, which was only safe because the grammar was checked per language first: gender survives peer/node → computer everywhere, but NOT device → computer in de/nl/ru/pl/cs; French and Dutch change the article; Turkish rewrites every suffix for vowel harmony; Czech `uzlu` covers three cases the target distinguishes. `\w` matches CJK and Devanagari so those needed unbounded matching, and three false friends had to be spared — Turkish `eşlenir` ("is mapped"), Portuguese `nós` ("we"), and the `{node}` placeholders (gotcha #611). Guard covers both words: `the_english_ui_uses_one_word_for_a_model_part_and_one_for_a_machine`. Original "shard" half: **DONE 2026-09-14 — the shard/part/piece split is closed; peer/computer is NOT.** The count was also wrong in two directions: there was a THIRD English word ("piece", 14 strings), so a search for "shard" reports the job two-thirds done, and there were four surfaces rather than one — the locale files, the markup's fallback text (which had drifted from `en.json` independently), the Rust `ActivityEvent` messages, and labels assembled in a variable and only later interpolated into one of those. The last group is invisible to any search of the emit sites and was caught by reading the page (gotcha #609). Guard: `the_english_ui_calls_a_piece_of_a_model_a_part`, which forbids both old words and allowlists the two strings meaning "pieces of work". **Still open: peer / node / computer / device**, 74 + 49 + 66 + 62 strings by the 2026-09-13 count — with the same warning as before, that half a rename reads worse than none, and with a short list that must stay technical (UPnP, the P2P port, a relay node). Identifiers, `shard_NNN.bin` and wire fields were deliberately NOT renamed and should stay that way. Original: | Counted 2026-09-13: 46 `shard` strings outside `activity.*` against 53 `part`, and 66 `peer` — so the split is UI-WIDE, not the feed-local jargon item the 09-12 sweep filed. One feed already contains both spellings two lines apart. **Rewording `activity.*` alone makes it worse**, converting a split feed into a split page; half a rename reads worse than none. ~112 strings x 21 locales, with a short list that must stay technical (UPnP, the P2P port, a relay node — the user has to act on those). R126's `enc.*` pass is the precedent for doing it in one go |
