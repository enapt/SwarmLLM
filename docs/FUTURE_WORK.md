...
| 43 | Phi-4-mini cannot serve a single request on the shard path | **Found 2026-09-11 by `examples/family_conformance.sh` on its first run covering that model.** Every request dies before a token with `Internal error: attn: slice-set only supports contiguous tensors`, raised from `SeqCache::append`'s `slice_set` in `inference/split/kv_cache.rs` via `forward_attn`. **The difference from Phi-3.5, which works, is GQA**: Phi-4-mini is `head_count = 24` / `head_count_kv = 8`, Phi-3.5 is 32/32, and both report `general.architecture = phi3` and ship a FUSED `attn_qkv.weight`. So a fused-QKV split that assumes equal Q/K/V widths yields a non-contiguous view for K/V, which `slice_set` refuses. Not a regression — nothing in v0.3.172 touches attention, and the same build serves Phi-3.5 cleanly. ⚠ **The field reports on this model came through `-m` (a whole GGUF via llama.cpp), which handles GQA itself, so their evidence says nothing about this path.** Reproduce: `examples/family_conformance.sh ./target/release/swarmllm 8824 microsoft-phi-4-mini-instruct-q4-k-m`. Read `docs/invariants/inference.md` before touching the attention path |
