...
|---|---|---|
| 90 | **The message dispatcher stopped consuming for 45 minutes and the node went on reporting itself healthy** | **FOUND 2026-09-18 in the live node's log; the cause is NOT yet established.** Between 11:36:10 and 12:21:51 the node wrote **230,402** consecutive `Dispatcher backpressured, dropping gossipsub message` WARNs — ~84/s for 45m41s, 74% of the whole log file. In that window there is **not one log line** with target `swarmllm::daemon::dispatch`, and not one `register_manifest` (which is emitted from inside the dispatcher's `ModelManifest` arm), so the `network_out` consumer (`daemon/mod.rs:340`, capacity 1024, consumed at `daemon/mod.rs:395`) accepted **nothing at all**. Other tasks were fine throughout: `PEX: dialed new peers` fired 115 times, the scheduler kept answering route previews, the credit ledger gossiped 7 times. **It did not recover** — it ended only because the node was restarted at 12:22:51 for the v0.3.188 deploy. Build was v0.3.187-alpha, uptime ~5h48m. **Why this matters more than the log noise**: that one channel carries gossip AND every inbound request/response message, so for 45 minutes the node could not receive a `LayerForward`, a `LayerResult`, a `StreamingToken`, a `RemoteGenerateRequest` or a shard announce. It was a black hole for the swarm while `/health/ready` answered `true` on every subsystem and local inference (which does not go through the dispatcher) kept working — gotcha #646's shape exactly, in a second subsystem. **Shipped in the same round, and deliberately NOT a fix**: `ChannelCounters::note_dropped` now rate-limits the report to one line per 30 s per channel and each line carries `nothing_accepted_for_secs`, so a recurrence is ~91 readable lines that name the stall instead of 230,402 that bury it. That makes the next occurrence diagnosable; it does not stop it. **What is NOT known**: which await, lock or blocking call wedged the task. **Two candidates were investigated and ELIMINATED on 2026-09-18** — correcting this entry's first draft, which listed both as live. ❌ **NOT a DashMap lock-order inversion.** `register_manifest` does hold its `manifests.get` guard across `has_origin_knowledge`, but that iterates `origin_verified`, a SEPARATE `DashMap`; two DashMap instances have independent lock arrays and there is no cross-map form of the documented recursive-`get` deadlock. No same-map recursion exists anywhere in the call graph, and no writer of `origin_verified` touches `manifests`. ❌ **NOT tokio `RwLock` write-starvation on `credit_balance`.** `CreditLedger::run` reads the IDENTICAL `Arc<RwLock<CreditBalance>>` (`daemon/mod.rs:432`) on its 300 s gossip tick, and `Gossiping credit balance` fired **9 times inside the wedge window** (11:38:32 through 12:17:20, verified by grep). tokio readers never block readers, so a reader stalling means a writer never released — and nine reads succeeded on that very lock while the dispatcher was stuck. Every writer (`ledger.rs:488,918,958`, `escrow.rs:443`) releases before any inner `.await`. ⚠ **The best-fitting mechanism found, still UNCONFIRMED for THIS incident**: `ModelProcessPool::cancel_request` (`process_pool.rs:5500`), called INLINE from the `CancelInference` arm at `dispatch/mod.rs:2593`. It loops over every live worker doing `worker.writer.lock().await` then `send_daemon(..).await`, **neither bounded**. It is the only inline un-spawned `.await` in the 2932-line dispatch file besides the two RwLocks, and every other site that takes `writer` (`process_pool.rs:1857,4808,4991,5176`) scopes the guard to a block and bounds the wait *specifically so a stalled worker cannot block the manager*. This one is the exception. A worker hung without erroring never sets `dead` (only a socket error does), so the `!dead` filter would not skip it. **Why unconfirmed**: the worker timeline for that boot is — spawned successfully 07:11:03 (loaded on CUDA), **idle-unloaded 08:13:14** (`idle_secs=3609`), respawn failed 09:22:44 (`worker connect timeout`, with an untimestamped `Inconsistency detected by ld.so: _dl_setup_hash: Assertion ... failed!` in the log — gotcha #647's shape, and #646's cause). So at 11:36 `self.workers` was very likely empty and `cancel_request` would have short-circuited at its `is_empty` guard. **It is a latent defect regardless** — an unbounded inline await in the dispatch loop is exactly what gotcha #74 forbids — and worth fixing on its own merits. `release_request_kv` (`process_pool.rs:5526`) has the identical unbounded shape but is called from a router task, not the dispatch loop. ⚠ **And the supervisor cannot see this failure class at all**: `daemon/supervisor.rs:66-101` reacts only when `subsystems.join_next()` RETURNS. A task parked for ever inside an `.await` produces no signal — zero `Subsystem exited` / `Critical subsystem failed` lines anywhere 11:00-13:00. The JoinSet watchdog catches panics and clean exits, never a hang. ❌ **NOT `pool_tx` starvation either** — its sole writer runs once at boot (`daemon/mod.rs:483`), before any of the 12 subsystems that could contend for it exist, so no later writer can starve a reader. ❌ **NOT a growth-based mechanism (candidate c)** — `peer_forward_counts` is swept on a timer, `retracted_claims` and `shard_holders` are capped, and the one known growth defect in this loop (the per-peer forward slot leaking on abort) already has its RAII fix; none of them stall the `select!` loop in any case, they cause a `continue`. ✅ **Shipped 2026-09-19: the next occurrence identifies itself.** `metrics.note_dispatch(msg.kind_name())` writes an epoch-millis marker and the message's variant name the instant `network_out_rx.recv()` returns, BEFORE the `match`, so it is on every path including the `continue`s; `HealthMonitor::report_dispatcher_stall` reads it on its own 30 s tick and logs at `error!` past 300 s idle, naming the last message's kind — i.e. which handler to suspect. **The two halves must stay in different tasks**: a heartbeat emitted from inside the dispatch loop goes silent for exactly the reason the loop is stuck, and `daemon::supervisor` cannot serve either because `join_next()` fires only on a panic or a clean exit. Guard: `the_dispatcher_liveness_marker_is_written_before_the_match_and_watched_elsewhere`, verified by planting a marker inside one match arm. **This does not stop the stall; it names it.** What remains open is the stall itself. **Do not "fix" this by enlarging the channel** — 1024 was not the problem; a consumer that accepts nothing fills any queue. ⚠ **A watchdog is the obvious next step and needs a decision**: the node can see this itself (the channel knows how long it has gone without accepting anything), but what it should DO about it — log, withdraw capacity, or abort and restart the task — is the same question #89 asks about the graphics card, and has the same false-positive cost. | Every inbound swarm message shares this one consumer, so a stall is a total outage that reports itself as healthy |
...
| 2 | Peer ranking uses ping, and a big payload under loss is not ping | Found from OUTSIDE by a contributor's netem lab (issue #21). **Both halves shipped** — the loss half as #495 (v0.3.164, made to actually fire 2026-09-08) and the goodput half as `GoodputEstimator` + `VertexCost::transfer_ms` — and **FIELD-VERIFIED 2026-09-10** on the reporter's re-run lab: at 8 KB the better-ping peer wins 3/3, at 513 KB the 3%-loss peer is 2.4-3.3x slower despite a 20 ms better ping, 3/3. The crossover is where retransmission overtakes RTT, i.e. exactly at hidden-state sizes. **Residual: the flip is verified in a shaped lab, not on the live swarm**, and the reporter's own harness had a broken sixth check they found and fixed before re-measuring |

### P2 — wrong behaviour, narrower or needing a decision first

| # | Bug | Why it ranks here |
|---|---|---|
| 89 | A node whose graphics stack died still advertises that it can serve models | **OPEN, raised 2026-09-18 by the fix that ships in v0.3.188-alpha; needs a POLICY decision, not an obvious fix.** When a graphics driver is updated under a running node, `libcuda.so.1` is orphaned for NEW processes while the daemon keeps the mapping it already has. v0.3.188 detects this (`gpu_support::gpu_runtime_has_failed`, set by `process_pool::diagnose_failed_start`) and **withdraws the GPU from the advertised capability** in `health/monitor.rs`, so peers stop routing GPU-sized segments here. **That is only half the truth.** This binary is LINKED against `libcuda`, so once the library is bad *no worker of any kind starts* — a CPU-only worker exits 127 exactly as a GPU one does (measured: `model-worker --help` under a deliberately corrupt `libcuda.so.1`). So the node goes on advertising that it can serve models on its processor, and it cannot serve anything until it is restarted. It CAN still serve shards, which need no worker at all (byte-range reads), and that is worth keeping — gotcha #646 makes the same point about leaving such a node up. **The decision**: should the latch also withdraw inference capacity (not just the GPU), keeping shard serving alive? That is a further change to what this node tells the swarm, which is why it was not done unilaterally at the .188 gate. Options: (a) leave it — peers fail one request each and re-route, and the node recovers by itself the moment a worker starts; (b) withdraw inference capacity while the latch is set, keeping shard serving; (c) treat it as a full inference outage and stop offering work entirely, which costs the shard serving too. **Weigh (b) against a false positive**: the latch is set only when a fresh `exec` of our own binary fails, which is specific, and it clears on any worker that starts — but if it ever mis-fires, (b) silently removes a healthy node from the swarm's inference pool until someone restarts it. Evidence and the reasoning behind the current half: `docs/invariants/memory.md` § "A worker that dies is not a worker that is slow"; gotcha #647; `memory/round_log_0918_gpu_health.md` |
...
| 81 | **On iOS/iPadOS Safari the whole page scrolled as one block** | **FIXED 2026-09-14** (report #034). Every long region already scrolled inside itself — `.container`, `.chat-messages`, `.session-list` — and `body` carried `overflow: hidden`, so the shell was *designed* not to page-scroll. Two WebKit behaviours defeated it, and they are entangled so neither could be fixed alone: `overflow: hidden` on `body` alone does not stop the page scrolling on iOS (the scrolling element is the documentElement), and `100vh` there means the viewport with the browser chrome COLLAPSED, so the shell rendered taller than the screen. Worse, Safari measures `dvh` against the LARGE viewport for as long as the page is itself scrollable — **so the lock is what makes the unit correct, not the other way round**, and a `dvh`-only fix would have done nothing. Now `html` and `body` are both locked to a fixed `dvh` height with `overflow: hidden` (`vh` first as the fallback), every `vh` length in both stylesheets is paired with a `dvh` one, and `overscroll-behavior: none` stops the rubber-band. A `touchmove` guard is deliberately NOT used: it is easy to write one that kills the inner scrolling the layout depends on, and with nothing taller than the viewport there is nowhere left to scroll to. Guard: `every_viewport_height_in_css_has_a_dynamic_fallback_beside_it`, verified to fail on a new bare `vh` AND on a removed `dvh` sibling. ⚠ **Verified by mechanism, not on the device** — no WebKit engine here. The null control reproduces the reported symptom in Chrome by removing exactly the two conditions (page scrollable + shell taller than viewport) and shows it gone with them; the dashboard's 2917px of content still scrolls inside a 931px container with the page locked. The reporter's iPad is where this actually closes |
| 82 | The iOS on-screen keyboard has never been exercised against the locked shell | **OPEN, not reported — raised by the #034 fix, 2026-09-14.** With the shell locked to `dvh` and the chat input at the bottom of `.app-layout`, focusing the input on iOS is the next thing a user does and nothing here has ever seen it. Safari's default (`interactive-widget=resizes-visual`) overlays the keyboard and pans the VISUAL viewport to keep the focused field visible, which should work — the page not scrolling does not stop Safari panning. The alternative is `interactive-widget=resizes-content` in the viewport meta, which shrinks the layout viewport so `dvh` (and therefore the shell) relayouts above the keyboard. **Deliberately not added**: it is a behaviour change on the one platform that cannot be tested here, and if the default already works it is a regression risk for nothing. Ask the #034 reporter whether the input stays reachable and correctly placed with the keyboard up before changing anything |
...
| 46 | A tool schema reached the model with its keys alphabetised | **FIXED 2026-09-12.** Every tool-rendering template was affected — Qwen3, Llama-3.1 and GLM-4 alike: a tool definition is a `serde_json::Value` by the time it reaches the renderer, `serde_json` was built without `preserve_order`, so its map was a `BTreeMap` and `{"type", "function": {"name", "description", "parameters"}}` came out `{"function": {"description", "name", "parameters"}, "type"}` — and minijinja's own map alphabetised again on the way into the template. `transformers`, vLLM (Python dicts) and llama.cpp (minja renders with `nlohmann::ordered_json`) all hand the model the author's order, so ours was the outlier, and there was direct evidence formatting is not cosmetic: changing only the SEPARATORS moved Qwen2.5-Coder-7B at `temperature: 0` from `<tools>` to `<xml>`. Fix: `preserve_order` on BOTH crates (`Cargo.toml` says why at each). Blast radius checked first — no signature or hash is computed over a serialised `Value` map, `TransactionReason` carries no `Value`, `Value` equality is order-insensitive under `IndexMap`; `json!` API responses now serialise in literal order, which is what OpenAI's own do. `sort_keys=True` in `tojson` became a real sort. Guard: `a_tool_schema_reaches_the_model_in_the_order_its_author_wrote_it`, verified to fail with the feature dropped from either crate. Evidence: `docs/invariants/api-surfaces.md` § "A model is told about its tools the way it was trained to be" |
| 48 | Nothing rebuilt the cloud routing catalog except someone opening the admin page | **FIXED 2026-09-11** (field-reported the same day). `provider_model_map` is what decides whether a cloud model is proxied or refused, and its ONLY writer is the `GET /api/admin/provider-models` handler — no startup fill, no timer. The destructive half is fixed: that handler used to `.clear()` the whole map and repopulate from whatever came back, and a fetch that TIMED OUT was indistinguishable from a provider with no models, so one slow provider erased its own entries and a bad minute erased every provider's. `merge_provider_catalog` now replaces per provider and leaves a non-answering one alone. The second half is now fixed too: `daemon::background::spawn_provider_catalog_refresh` re-asks every configured provider every 15 minutes (first pass 20 s after boot), so a node whose admin page is never opened still routes its cloud models. **Verified firing on an isolated node** — `provider catalogue refreshed fetched=6 routable=2` with no dashboard open. Failure stays non-destructive because the per-provider merge above leaves a non-answering provider's entries alone. ⚠ The advertised list legitimately contains MORE than the routing map: Anthropic, Claude-subscription, Moonshot fallbacks and custom providers are static entries that route by `provider:model` syntax or the `claude-` prefix without the map, which is why `fetched` exceeds `routable` and is not a defect. Field evidence: `cloud_models_available` read 0 for days while the dashboard still listed 80 models — the display cache keeps its previous answer on a failed refresh and the routing map did not, so the two can disagree completely |
| 49 | Auto-prune evicted shards the swarm had no other copy of | **Field-reported 2026-09-11, NOT reproduced, do not fix on this reasoning alone.** A node with `max_storage_mb` set just under what it held (8000 vs 7839) sat in a continuous urgent-pressure prune loop and ended up missing shard 0 of gemma-2-2b-it and 4 of 5 of phi-3.5-mini, on two models it was recorded as the sole replica of; re-acquisition then looped `stalled shard download … stall_secs=30` against two peers. Prune's redundancy check already filters holders to CONNECTED peers (the fix for the offline-holder over-count), so the surviving hypothesis is a holder that is connected but no longer holds the shard — the add-only DHT provider record that outlives the fact by up to 24h, see `docs/invariants/network.md`. ⚠ **The stall does not fit that cleanly**: a peer asked for a shard it lacks answers `ShardResponse::empty()`, and the requester fast-fails on it (`retry_shard_or_fallback`), so a 30s stall means those peers did not answer AT ALL, which is a different mechanism. **The discriminating evidence is one line from the reporter's log** — whether `Peer returned empty shard data` appears before the stalls or nothing from the peer does. ⚠ **Half-answered from OUR OWN node 2026-09-14, which shifts the prior**: this node's log covers six days at INFO and holds 14 `stalled shard download` lines and **zero** `Peer returned empty shard data` — a `warn!`, so its absence is real evidence, not a level artefact (diagnosis rule 2). Tracing one stall end to end (2026-09-11 22:19:31.562 `Sending shard transfer request` → 22:20:08.369 stall, 36.8 s) there is **nothing whatever** about that peer in between: no response, and no `OutboundFailure` either, though the log carries 248 of those elsewhere. So the stall mechanism HERE is a request that vanishes, not a peer answering empty — the same silent-drop behaviour `RR_ACK_TIMEOUT_SECS` exists for on the `SendDirectMessage` path, which the shard-request path does not use (it relies on the 30 s stall watchdog instead). That is evidence about THIS node's peers, not the reporter's, so the ask still stands — but the stale-DHT-provider→empty-response story now needs positive evidence rather than being the default. Ask before building anything. The asymmetry that makes this worth chasing: for a destructive decision an over-count is unsafe and an under-count merely conservative, and prune currently treats a holder claim as proof of redundancy. ⚠ **Two things changed under this entry on 2026-09-17 and both move its priors.** (a) One source of false holder claims is closed: a peer was recorded as holding a shard when its download reached 100% of BYTES, before the hash check, so every failed verification left a holder claim behind — see `docs/invariants/network.md` § "A holder claim means VERIFIED". The surviving stale-DHT hypothesis now has one fewer rival. (b) Disk pressure counts ~5% more than it did, because the budget now measures the directory rather than the manifest — so a node sitting just under its limit, which is exactly this report's 8000-vs-7839, arms urgent pressure SOONER than it used to. The reclaim pass that ships with it is what stops that becoming a floor it cannot prune below, and **that pairing is the thing to check first if this symptom is reported again.** |
...
| 65 | The UI used four words for two things | **DONE 2026-09-14, both halves.** A piece of a model is a **part**; a machine is a **computer**. The pool feature keeps **device** — the user's decision, asked because the answer decided the target word: machines you own and link are a different idea from anyone's computer on the swarm, and "My Devices" is a nav destination named for the first. Guard: `the_english_ui_uses_one_word_for_a_model_part_and_one_for_a_machine`, which forbids "shard"/"piece"/"peer"/"node" in `en.json` and allowlists the two strings meaning "pieces of WORK". **Two things this cost that the estimate did not include.** There was a THIRD English word for a model piece ("piece", 14 strings), so searching for "shard" reports the job two-thirds done. And the vocabulary lives on **five surfaces**, only the first findable by that search: the locale files; the markup's `data-i18n` fallback text (which had DRIFTED from `en.json` independently); Rust `ActivityEvent` messages; labels assembled in a variable and only later interpolated into one; and a helper that RETURNS the word (`ShardId::display_index`), which also explained a never-reported on-screen defect — "P2P: downloading shard shard 15 from peer", two callers writing the noun the helper already supplied. Gotchas #609/#610. **Translations were mapped, not retyped**, which was only safe because the grammar was checked per language first: gender survives peer/node → computer everywhere but NOT device → computer in de/nl/ru/pl/cs; French and Dutch change the article; Turkish rewrites every suffix for vowel harmony; Czech `uzlu` covers three cases the target distinguishes; `\w` matches CJK and Devanagari so those need unbounded matching; and three false friends had to be spared — Turkish `eşlenir` ("is mapped"), Portuguese `nós` ("we"), and the `{node}` placeholders (gotcha #611). Identifiers, `shard_NNN.bin` and wire fields were deliberately NOT renamed. Evidence: `docs/invariants/frontend.md` § "One word per thing" |
| 62 | The Network map is blank on a node that has not served across regions | **Honest half shipped 2026-09-13** — the map now has a key for its arcs (which had none, in either colour) and says, on a node that has run nothing across regions, that lines appear once you chat with a model elsewhere or serve one for someone. Seeding from the swarm's traffic is still open and still needs a gossip message plus a privacy decision. Original: the arcs are drawn from THIS node's `recent_traces`, so a new user — the audience the dashboard is being tuned for — opens the Network tab and sees pins and no traffic. Honest, and the opposite of welcoming. Either seed from the swarm's routes (needs a gossiped route summary, and a privacy decision: it exposes who served whom) or leave it and accept the quiet map. Raised with the user 2026-09-13, not resolved |
| 63 | The per-model activity ticker lived inside the expanded model card | **FIXED 2026-09-13.** A log inside a card, and a second rendering of what the Activity panel already held — every event reached both, and the ticker kept its own store, signature cache, expand handler and "ago" refresh loop for the copy. `App.notifications.filterByModel` now narrows the Activity AND Network panels to one model and the card links to it. Three things that had to be right: the filter belongs in the render SIGNATURE (which skips on `topTs + length`, neither of which a filter changes — gotcha #585's shape), the count must be taken after filtering or it describes a different list from the rows, and a filtered log with no matches must not say "no activity yet" as though the node were idle. `activity.count` also gained plural forms: it could always print "1 events", and filtering makes that common |
| 61 | A disputed shard is kept but the disagreement is never settled | Fallout of item 60, and strictly better than what it replaced (data loss). A shard disagreeing with an unbacked hash is kept and served, but nothing resolves which side is right. `mark_shard_for_repair` is NOT the answer — `complete_pending_shard_fetches` clears any mark whose file is on disk, which is every shard on this path. **The instrument shipped 2026-09-13** (`state.models.disputed_shards` + a diagnostics section printed even at zero + a dashboard badge + `disputed` in the shard listing), which also fixed two reporting defects: a KEPT shard was announced to its owner as a red "failed its integrity check — re-downloading automatically" (nothing is queued, nothing is wrong, and the advice invites deleting a possibly-last copy), and the startup sweep's summary omitted disputes entirely so a node keeping disagreeing bytes said "Verified 20 shards". **Settling it is still open** — three designs costed in the entry body, none to be built before there is field data. ⚠⚠ **The first reading was taken 2026-09-14 and then WITHDRAWN — the instrument was incomplete.** The live node reported ZERO across 65 held shards, and that was recorded as a measurement. Hours later an isolated node logged `A shard we are serving disagrees with the hash the swarm reports — keeping our bytes` **twice**, for two shards of llama-3.2-3b, while its own diagnostics said "shards kept despite disagreeing (0) — none, every checked shard matches its expected hash". **Three paths compute `mismatch_policy` and can land on `KeepBytes`; only two recorded it.** The third — `auto_manage::manager::verify_pending_shards`, which drains `shards_pending_verification` — warned and returned. It is reached by exactly the nodes most likely to be in this state: ones whose manifests came from peers rather than from an origin, so `register_manifest` had nothing origin-backed to refuse the peer's claim with. Fixed the same day, with `clear_shard_dispute` added on that path's success arm too (a clear that has to be predicted is a clear that gets forgotten). Guard: `every_path_that_keeps_disagreeing_bytes_records_the_dispute`, null-control-verified. ✅ **And verified END TO END on a live node, with the mechanism observed firing rather than inferred**: the same isolated node, rebuilt with the fix, logged the `auto_manage::manager` warning for llama-3.2-3b shards 0 and 1 at 16:35:37/38 and its diagnostics then named exactly those two — `shards kept despite disagreeing (2)`. Getting there needed the condition re-armed, which is itself worth writing down: the path fires only when a held shard's EXPECTED hash *changes*, so a restart with the peer's hash already persisted never reaches it; `db.redb` has to be deleted while the node is STOPPED. **What is still true and is the useful half:** the live node's log holds **1503** `Ignoring a shard hash that contradicts the one we took from the origin` warnings over six days across 8 models, so the disagreement is pervasive; wherever a node holds origin provenance the .177 gate refuses the claim one step earlier and no dispute is created, which is why an origin-provenanced node can legitimately read zero. A fresh reading is needed after the fix ships, and **it must come from a node whose shards arrived from peers** — a reading from this node cannot distinguish "no disputes" from "the path that would record them is not reached here". Settlement stays parked until then. Lesson: a count that reads zero while the thing it counts is happening is worse than no count, because it is read as evidence (diagnosis rule 2) |
...
| 75 | **The Settings panel wrote its markup defaults over the user's config when it could not read it** | FIXED 2026-09-14, found chasing #69's recorded residual. `settings.load` returned early on a failed config fetch, leaving the panel at its static HTML defaults (10 requests, unlimited bandwidth, 50 GB of disk); `settings.save` then read all eight fields out of the DOM and PUT every one, so changing one setting during a 401 or a 503 overwrote the other seven — under a toast saying "Settings saved". `PUT /api/admin/config` was **already** written for partial bodies (it builds on the live config and applies only named fields), so the whole loss came from the client always sending a full document. `save` now sends only what differs from what `load` put in the form, a form that was never populated cannot be saved at all, and the Save button starts disabled with `settings.unreadable` explaining why. Verified on an isolated dev node, with the null control: the node held `max_disk_mb = 120000` and the old save would have written `50000`. Gotcha #595 |
| 76 | **Any settings change not saved within thirty seconds was silently thrown away** | FIXED 2026-09-14, found immediately after #75 in the same panel. `dashboard.js::loadInitial` was a second writer of four Settings fields, and `notifications.js` runs it on a **30-second poll** — so it fired while the panel was open and reverted whatever the user had dragged. Measured before and after: drag Max Disk 120 GB → 300 GB, wait one poll, slider reads 120 GB again (before) / 300 GB (after). To a non-technical user this is "the setting will not stick", with nothing logged. `App.settings.load()` already populated all four on every open, so the dashboard copy was redundant and there were no other readers. Gotcha #596 |
...
| 80 | **A reasoning model's whole `<think>` scratchpad still streamed to the user — the #031 filter only ran for requests carrying `tools`** | FIXED 2026-09-14, field report #032 against **v0.3.179, the release that shipped the #031 fix**. `StreamingToolText` does two jobs behind one buffer: hold text back while it could still be a tool call, and withhold a reasoning preamble. The second was added later, and all four streaming surfaces wrapped `push` in `if tools_requested` — so on an ordinary chat message, which carries no tools and is nearly all real traffic, the filter was never called and the raw text went out. **Reproduced and controlled on the released binary**: same model (qwen3-1.7b), same prompt, one field different — without `tools` the first delta was `{"content":"<think>"}` and the whole scratchpad streamed; with them the reply was `"C" "iao" "!"`. Non-streaming was never affected (`finalize_reply_text` is called unconditionally at all three text sources). Fixed at the choke point: `StreamingToolText::new(detect_tools)` with **`Default` removed**, so no call site can avoid stating which kind of reply it reads and the compiler finds them all; `push` always filters, `releasable_len` withholds for tool detection only when asked (ordinary chat still streams token by token); both flush helpers return early with `pending_all()` when `!detects_tools()` so prose is never parsed into a false `finish_reason: "tool_calls"`; both end-of-stream flushes unconditional. ⚠ **The #031 tests could not have caught this** — every one called `push` directly, which is what production did *only on the branch not taken*. The equivalence test now runs `detect_tools` both ways, and a null control confirms both new assertions go red without the fix while all nine pre-existing ones stay green. Gotcha #601. ✅ **Re-confirmed on the RELEASED .181 binary 2026-09-14**, which is the artifact users are running: same model (qwen3-1.7b), no `tools`, `stream: true` — first content delta `"C"` then `"iao"`, no `<think>` anywhere. The reporter has not come back, so this is our own confirmation on a shipped build rather than theirs, and it is worth having: the .179 fix for the sibling report looked right from the inside too |

### Fixed after v0.3.181-alpha (2026-09-14)

| # | Bug | What it cost |
|---|---|---|
| 83 | **A model card counted computers that could never send you the file** | FIXED 2026-09-14, found by reading the live node's own log rather than from a report. `ModelRegistry::shard_holders` has dropped holders claiming a different GGUF build since 2026-09-05 (gotcha #406) — a model id comes from a display name, so independent builds collapse into one identity and a peer on another build holds bytes our hash check would refuse. `all_shard_entries` returns the map RAW, and its doc note permitted a consumer that renders "a count for a human". **That permission was the hole**: the count picks the model health sentence a non-technical user reads to decide whether the model keeps working (`dashboard.say_safe` / `say_at_risk` / `say_only_you`), writes the "on N other computers" line, and decides whether a model is listed at all. Two writers had it wrong at once — `admin_models::listing` (`peers_hosting`) and `websocket::build_stats_message` (the per-shard `holders` in the 2 s `stats_update` tick) — **and they write the SAME dashboard row**, so the number depended on which touched it last. **Measured on the deployed .181 binary, both payloads taken inside one tick: 36 of 97 shards disagreed, 0 in the impossible direction** (raw ⊇ filtered, so a filtered count can never exceed a raw one — a first attempt with snapshots minutes apart showed 57 disagreements in BOTH directions, which was drift, not the bug; re-taking them together is what made the figure mean anything). Worst case `qwen2.5-14b-instruct-q4-k-m` shard 11: 1 via the page load, 4 via the live tick. The swarm context: 1503 `Ignoring a shard hash that contradicts the one we took from the origin` warnings across 8 models in six days, and for `qwen2.5-coder-7b-instruct-q4-k-m` two peers announced parts of it while neither appeared as a holder — the same peers ARE holders for six and eight other models respectively, which controls for timing and for the peers themselves. **The dropped peers are now reported rather than silently subtracted** (`peers_other_build` beside `peers_hosting`, `ModelPeerCounts` carrying the pair so a call site cannot supply one alone, and a card line in 21 languages), because a count that quietly shrinks is worse than one that is too high. Deliberate exception documented at `admin_hf::count_unique_shard_holders`, which answers "how many copies exist in the swarm" for a model not yet downloaded. Guard: `a_holder_count_shown_to_a_person_is_the_count_that_can_serve`, verified to go red on the real pre-fix `websocket.rs`. Evidence: `docs/invariants/network.md` § "A holder record names a BUILD" |
...
### The storage budget cannot see about 5% of what the node puts on disk (2026-09-15; FIXED 2026-09-17)

**Superseded — read this first.** Fixed 2026-09-17, both halves together, which
is what the entry below predicted it would take.

- **`auto_manage::held_disk_bytes`** measures the models DIRECTORY, and
  `storage_budget_now` and `scoring::remaining_budget` both report it. The count
  still comes from the registry, because a shard count answers a different
  question and pricing it off the directory would count a header as a shard.
- **`shard::cleanup_orphaned_model_files`**, called once per prune cycle from
  `prune::reclaim_orphaned_model_files`, gives back `gguf_header.bin`,
  `tied_output_weight.bin` and `mmproj.gguf` for any model this node no longer
  holds a part of.

⚠ **The second half is not optional, and shipping the first alone would have
been a regression, not a fix.** Prune deletes `shard_NNN.bin` and nothing else,
so charging disk pressure for files it cannot free leaves a node near its limit
shedding shards forever chasing a floor it can never reach — which is the shape
of item 49, the sole-replica prune report. Anyone tempted to simplify this back
to one change should read that entry first.

Losing the derived files is cheap where it is not free: `tied_output_weight.bin`
was 1566 MB of the 1637 MB gap measured here, and it is EXTRACTED from
`shard_000.bin` by `daemon::manifest::extract_tied_output_weight` and
re-extracted automatically at startup. The header is ~6 MB and re-fetched on the
next attempt.

Three guards, all required because this deletes files, each null-control
verified to go red when removed: the registry must record no held part (which
includes the `MMPROJ_SHARD_INDEX` sentinel, so a node holding only a vision
encoder is skipped), no `shard_NNN.bin` may remain on disk, and no shard of the
model may hold a download claim. `manifest.json` and `hf_source.json` are kept.

⚠ **Found on the way, and it was the more dangerous bug:** the auto-manage test
fixture never set `config.node.data_dir`, so `Config::default()` pointed every
unit test at the developer's REAL `~/.local/share/swarmllm`. Harmless only while
nothing under test touched the filesystem — the moment the budget measured a
directory, a test asserting on a node "holding 14 GB" read this machine's actual
33 GB instead. The fixture now owns a temp dir, and `write_sparse_shards` gives a
test real holdings via `set_len` at no disk cost.

---

**Original entry, 2026-09-15 — found while exercising the download/cancel path.**
Recorded with the measurement so the next round starts from data.
...
| # | Bug | What it cost |
|---|---|---|
| 71 | **A shard nobody could date lost the protection against being pruned minutes after it arrived** | FIXED. `prune.rs`'s "recently acquired" signal (−0.2, a 30-minute anti-thrash window) was `metadata().and_then(modified).ok().and_then(\|t\| t.elapsed().ok()).map(..).unwrap_or(false)`, which reads THREE different unknowns as "this shard is old": an unreadable mtime, any other read error, and — the one worth naming — `elapsed()` refusing a timestamp in the **FUTURE**, which is exactly what clock skew or a copied file produces. This score decides DELETION, and the asymmetry that governs every destructive decision in this codebase applies: treating an unknown as old withdraws the protection, while treating it as recent only delays a prune by one cycle. Now `recently_acquired_from`, a pure function with all four branches tested — `NotFound` is the one case where "not recent" is right, and it is moot because there is nothing left to prune |
| 73 | **"Used recently" was asked of the one signal a local request never writes** | FIXED 2026-09-14, found by audit. R134.7's prune protection (`RECENT_REQUEST_PENALTY`, −1.5 for a model used inside the window) read `model_trust.last_request_at` under a comment saying it is "already updated per request". `ModelTrustInfo::record_request` has **exactly one caller in the tree** — `router::distributed_exec` — so a node serving its own complete local model takes `local_fast_path_for`, skips the router, and never writes it; serving a peer does not write it either (the blind spot documented on `serving_models`). **On a single-node install — the shape "new users first" targets — the protection never fired at all**: a shard that answered thirty seconds ago was scored for pruning exactly like one nobody had touched in days. ⚠ **The fix already existed and had been applied to the sibling consumer only.** `effective_idle_secs` was given all four signals on 2026-09-02 after a stale `last_request_at` unloaded a model five seconds after it answered (gotcha #437); its doc comment says in terms that "the local split path — the common one — writes neither". That landed on the idle-VRAM-unload path and not on this scorer 400 lines below it. Now both combine the same four (router request, peer-served, worker `last_used`, residency as an upper bound) |
| 74 | **The rebalancer counted holders it could not fetch from** | FIXED 2026-09-14, found independently by two auditors. `ModelRegistry::shard_holders` drops holders that positively claim a different GGUF build — their bytes would fail our hash — and is documented as the single read accessor. `all_shard_entries()` returns the map RAW, and `health::rebalancer::find_underreplicated_shards` counted `holders.len() < MIN_REPLICATION` from it. A shard with one real copy plus one build-conflict holder therefore read as replicated twice and never triggered re-replication — precisely the case that function exists to catch. Not the #384/#581 data-loss class: the destructive path (`prune::can_reacquire`) already receives filtered holders, so this was a missed detection, not a deletion. The other twelve `all_shard_entries` consumers narrow to `holders.contains(&local_node_id)` (unaffected — a node's own registration always matches its own build tag) or render a count for a human; the accessor now says so, and says that anything deciding on the count must re-ask |
...
| # | Bug | State |
|---|---|---|
| 66 | **`manifest.json` is read-modify-written by ≥10 call sites; the CORRUPTION half is fixed, the LOST UPDATE is not** | **Corruption FIXED 2026-09-14.** Each save now stages under its own name (`manifest.json.<pid>.<n>.tmp`) behind a lock, so two concurrent writers can no longer interleave into one staging file and promote a mix of two JSON documents — which meant a manifest that would not parse, and a model skipped entirely at startup. Reproduced deterministically by `concurrent_saves_never_leave_an_unparseable_manifest` (ten threads, twenty saves each): with the shared name restored, threads panic on both the save and the read-back. ⚠ **The lost UPDATE is NOT worth the refactor the audit implied, and this is the second severity corrected down in this round.** Traced 2026-09-14: the disk copy losing a hash is ALREADY covered, by design, and the design is written down at `model/registry.rs:343` — *"`load_from_db` is what repopulates this registry at boot, BEFORE the local disk scan, and the disk copy is exactly the thing that carries placeholders… the merge above already stops the disk copy's placeholders overwriting anything."* `startup.rs:32` loads redb before the disk scan at `startup.rs:247`, and `register_manifest`'s `merge_known_shard_hashes` refuses to let a zero overwrite a known hash. The two call sites that write the DIR but not redb both turn out to be safe: `model/shard.rs` only recomputes `manifest_hash`, which is derived from the rest of the content and recomputed on the next scan; and `admin_hf/shards.rs` builds a fresh manifest whose zeros are recovered by `register_manifest` immediately after, which fires the persist hook (`recovered > 0 && changed`) and so puts the MERGED version into redb. **And the obvious alternative fix is actively unsafe**: making `save_to_dir` merge from what is on disk would let a stale hash land in the zero slot of a shard that is ABSENT, and the P2P accept path would then reject the good bytes when it finally arrives — gotcha #384's class. So: closed in practice. Reopen only with evidence of a hash that survived nowhere. `ModelManifestExt::save_to_dir` (`src/model/manifest.rs:241-248`) stages to the same tmp name for every caller. At least ten paths do `get_manifest()` → mutate one field → `compute_hash()` → `save_to_dir()` → `register_manifest()` independently — the HF per-shard hash update (`auto_manage/download.rs:586`, **one per downloading shard**, each in its own `tokio::spawn`), the P2P landing (`model/shard.rs:425`), the admin download (`admin_hf/shards.rs:952`), `acquisition.rs:344`, `scan.rs:293`, `daemon/manifest.rs:149/177/342`, `repair.rs:158`. `download.rs:237`'s own comment says "Multiple shards of the same model may be downloading concurrently", so two shards finishing seconds apart is the everyday case. **The in-memory registry is protected** — `register_manifest` runs `merge_known_shard_hashes`, which cannot lose a known hash — **but `save_to_dir` bypasses that merge, so the on-disk file is not.** Task B, holding a snapshot taken before A registered, writes its version over A's, silently reverting shard A's origin-verified hash to its old (often zero) value. `verify_hash()` still passes, because each writer's embedded `manifest_hash` matches its own content, so it does not read as tampering. On the next restart the BLAKE3 sweep then compares correct bytes against the reverted hash. ⚠ **Plausible independent mechanism for the "kept shard shown as a red integrity failure" symptom of .179 — NOT proven to be the same cause.** Fix shape: a per-model lock plus a unique tmp name (the `.tmp` fix of 2026-09-14 is the precedent; `huggingface_hub` PR #4306 is the prior art for why unique names beat locks) |
| 67 | **The self-update staging file is shared between the daemon and a manually-run `swarmllm update`** | ⚠ **Severity CORRECTED DOWN 2026-09-14 after reading the code the audit reported it from — it is already mitigated, and the fix it suggested would break the feature.** The shared name is deliberate: `download_update` stages to `<binary>.update.tmp` and a LATER, SEPARATE request (`POST /api/admin/update/apply`, `api/admin_providers.rs:1130`) finds it by exactly that name, which is also what `info.downloaded` reports. A per-process name would sever that handoff. And `apply_update` **re-hashes the staged file immediately before the rename** — its comment names this exact threat ("a local process can swap the staged file during the gap") — so a file corrupted by a second writer is REFUSED, not applied. The real residual is therefore a wasted download and a confusing "SHA256 mismatch" rather than the bricked binary the audit projected; `download_update` also reuses an already-staged verified file, so the daemon does not race itself. Genuine remaining gap: a write landing in the microseconds between that `std::fs::read` and the `std::fs::rename`, which no re-check can close and which needs cross-process exclusion (a lock file, with the stale-after-crash problem `filelock`'s `SoftFileLock` solves via a process-liveness record). Not worth building on this evidence. **The lesson is the entry, not the bug: an audit's severity is a hypothesis.** | MEDIUM-HIGH as filed; LOW in fact. `update.rs:806` stages to `binary_path.with_extension("update.tmp")`, a path derived only from the binary's location — identical whether computed by the daemon's background `UpdateChecker` or by `cli/update.rs:22-26`, which builds its **own** `UpdateChecker` in a **separate OS process** rather than asking the running daemon. No pid suffix, no lock. Reachable when `auto_update` is Download/Install (not the default, but a normal choice) and the operator runs `swarmllm update` while the background loop is mid-download — a ~1 GB transfer, so a window of minutes. The existing re-hash-before-rename fix closes a single call's own TOCTOU; it does not stop a second process rewriting the file during the first's download. Worst case is the same "bricked node" shape as the 0.3.57→.58 incident documented in that file, via a different trigger |
| 68 | **A dashboard settings save silently discards a hand edit to `config.toml`** | **The worse half is FIXED 2026-09-14, and the audit had not spotted it**: the save was a bare `std::fs::write`, which truncates first and fills after. `Config::load_or_create` returns `SwarmError::Config` on a file it cannot parse — correctly — so the daemon **refuses to start**, and a crash, a kill or a full disk during a settings save therefore left a non-technical user with a node that would not come back up. It now stages under a pid-suffixed name and renames. ✅ **The clobber is FIXED 2026-09-17.** `base_for_partial_update` reads `config.toml` and applies the request's fields to THAT, falling back to the live config when the file is absent or does not parse — a broken file must not cost the user the change they just made in the dashboard. **The reconciliation this entry called a design question does not exist**: `apply_live_config` has exactly ONE production caller besides `reload_config`, and it is this handler (the other two call sites are tests). So nothing changes the live config outside the document, the only way file and live config can diverge is a hand edit, and re-reading is therefore strictly correct rather than a trade-off. Consequence worth knowing: a hand edit now takes effect on the next dashboard save rather than waiting for a `/reload` — which is what taking effect instead of being destroyed looks like. Null-control verified: the hand-edit test goes red against the old `cfg()` base. Original severity MEDIUM, silent loss rather than corruption. `PUT /api/admin/config` (`api/admin.rs:1262-1432`) builds the new file from `state.shared_state.cfg()` — the live in-memory config — never re-reads the file, and `std::fs::write`s it directly (no tmp+rename). Editing `config.toml` by hand while the daemon runs is the normal way to add e.g. `bootstrap_peers`; if the operator then changes one setting in the dashboard without `/reload`, the save overwrites the file from the stale snapshot and the hand edit is gone with no warning. Silent loss rather than corruption, which is why it ranks below #66/#67 |
| 69 | **The frontend collapses a failed fetch into "there is nothing here"** | **FIXED 2026-09-14** for the three consumers that acted on it; the mechanism is now in place for any other. `App.data.loadReachedDaemon(key)` records, at the single data-fetch choke point, whether the most recent load actually reached the daemon — `fetch` does not reject on an HTTP error status, so a 401 from a rotated key, a 500 or a 503 used to arrive indistinguishable from "this node has nothing". `models.js` now leaves the page alone when nothing came back AND the daemon was not reached, instead of replacing the model list with the first-run empty state and calling `updateChatAvailability(false)`, **which disabled the chat box on a fully-configured node**. `swarm-tab.js::_browseSearch` throws on a non-OK response so it reaches the `.catch` that already says the right thing (its sibling `_browseEnsureTrending` had this fix; it was never carried down). `settings.saveManualKey` answers `'ok'`/`'rejected'`/`'unreachable'` rather than a boolean, so **only a 401/403 is reported as a wrong key** — a correct key pasted while the node restarted was told it was rejected, and the input cleared, on the one screen someone reaches before anything else works. ⚠ **The residual was not only cosmetic** — auditing the rest of the consumers found the Settings panel WRITING its markup defaults back over the user's config (item 75), which is the same "a failed fetch became a value" mechanism with a destructive consumer instead of a misleading one. **Check what a consumer WRITES, not only what it renders.** ✅ **Second half done 2026-09-14**, and it was a worse bug than the remaining renderers: `core/data.js` was ASSIGNING the failed result to `App.data.cache.*`, so one transient 401 or 503 did not merely mislead one component — it **destroyed the last good value for the twelve that read the cache directly** (header status strip, swarm tab, network map, encryption toggle, `utils.js` model-name helpers, `models.js` peer count). `notifications.js` merges the WebSocket tick INTO `cache.stats`, so a nulled cache also dropped every field that tick does not carry, and `ui.js::networkStatus.load` reads `cache.stats` FIRST — rendering "0 peers, no local model" from a fetch that never reached the daemon. Each cache write is now gated on `loadReachedDaemon`; the helpers still RETURN what the fetch produced, so callers distinguishing empty from unreachable are unaffected. Guard: `a_failed_fetch_never_overwrites_the_frontend_cache`, null-control-verified (planting one ungated write names its line number). ⚠ **Remaining**: the other `load*` consumers still render their ordinary empty state on a failed fetch. They no longer have to — `loadReachedDaemon` is there — but each empty state needs its own honest wording, and that is i18n work across 21 locales. Original finding: HIGH for user impact, frontend-only. `frontend/js/core/data.js:67-131` — every `load*` helper does `try { if (r.ok) X = await r.json(); } catch(e) {}` and returns the `[]`/`null` default on both a non-2xx response and a thrown error. `fetch` does not throw on HTTP error status, so a 401 (expired key), 500 or 503 falls through with no signal at all. This is the choke point `.claude/rules/architecture.md` § "Frontend Data Fetching" tells every component to use, so every caller inherits it. Concretely: `models.js` computes `hasAny` from `loadModels()` and calls `updateChatAvailability(false)` — **a transient 401 disables chat and shows a fully-configured node the first-run onboarding state**; `dashboard.js`'s `loadNetworkData` has a `catch` written for exactly this failure that is **dead code**, because `loadPeers()` never rejects, so a failed peer fetch renders the ordinary "no peers yet — share your Peer Address" advice. Two siblings of the same shape: `swarm-tab.js:589` `_browseSearch` renders a 502 from HuggingFace as "no results found" (the function directly above it, `_browseEnsureTrending`, was already fixed for this in the 09-13 round and the fix was not carried down), and `settings.js:442` `saveManualKey` returns `false` on any non-OK, so a **correct** API key entered during a transient 500 is reported to the user as rejected. Fix shape: the helpers must distinguish "the daemon said nothing" from "we could not ask it" — the `giveBack`/`onFail` pattern already used in `_restFetch` |
| 85 | **A distributed pipeline never checks the prompt against the model's context window** | ✅ **REPRODUCED 2026-09-18 — the entry's "establish what actually happens" is now answered, and it is worse than the three guesses.** It does not error early, silently truncate, or degrade: it **accepts the request, generates for as long as it takes to reach the window, and then fails the whole thing with HTTP 400 `invalid_request_error`**. Non-streaming, everything generated is DISCARDED — the caller waits 40 s and gets nothing. Streaming keeps what was already sent and then emits a proper `StreamEvent::Error` (correct per `a_stream_that_fails_never_pretends_the_model_chose_to_stop`), so the reply simply stops mid-word under an error. **The message misattributes the cause**: "This conversation is 260 tokens, longer than the 256 this model is currently set to serve … send a shorter prompt or a smaller max_tokens" — the caller sent a 38-token prompt, and 260 is the prompt plus what the MODEL chose to generate. A 400 blames the request, which the server itself accepted 40 seconds earlier. **Where it comes from**: not the missing check, but `inference/split/executor.rs:461`'s per-forward pre-flight (`index_pos + seq_len > self.max_seq_len` → `SwarmError::Validation`), which is right for a prompt that is too long at prefill and wrong for generation reaching the wall. **Contrast, same request, same model**: the local path refuses in advance via `resolve_max_new_tokens` with "38 tokens of prompt plus 9000 you asked to reserve for the reply is 9038, and the model's limit is 2048. Ask for at most 2010 reply tokens" — accurate, actionable, before any work. **The fix this points at** is NOT the pre-check the title asks for (the coordinator still cannot know the effective window, and a conservative check against the DECLARED one would wrongly refuse anyone who raised `max_seq_len_override`, the very setting the error text tells them to raise). It is that **a generation which reaches the context window has finished for length, not failed**: `finish_reason: "length"` is what the OpenAI surface already has for exactly this, and what `resolve_max_new_tokens` already does on the local path by lowering a non-explicit budget to `room`. Shape: give the window-reached case its own `SwarmError` variant at the `executor.rs` site (per `completeness.md`, "give a permanent failure its own variant rather than filing it under `PipelineError(String)`" — and do NOT match its prose, gotcha #295), carrying `used` and `window`; `pipeline/distributed.rs`'s loop then ends the reply with `length` when tokens have already been produced, and only a prefill-time overflow stays a 400. ⚠ Keep the prefill case a 400 with the existing wording — it is correct there. **Reproduction, ~10 minutes, no GPU**: two nodes on one private `gossip_network_id`, each holding half of TinyLlama (copy `manifest.json` + `gguf_header.bin` + `shard_000.bin` to one and `shard_001.bin` to the other — 22 layers, shard 0 = layers 0-12), `auto_manage.enabled=false`, `prune_enabled=false`, `disable_default_bootstrap=true`, mDNS on so they find each other; confirm `x-swarm-route: distributed` and `x-swarm-segments: 2` on a short request first. Then set `inference.max_seq_len_override = 256` on both and send a 38-token prompt with `max_tokens: 9000` — generation crosses at token 258. ⚠ **Ports must be ≥11 apart**: P2P TCP is API port + 10, so a second node on 8821 collides with the first node's P2P listener and dies with `Address already in use`, which reads like something else entirely. | Severity now known, and higher than "UNKNOWN": a complete reply destroyed, reported as the caller's mistake. Still narrow — it needs a model split across nodes AND a reply long enough to reach the window
| 86 | **`SwarmError::PipelineError` answers 500, so "the swarm has no route for this model" is reported to the user as a crash in the node they are talking to** | **FOUND 2026-09-18 while fixing the pipeline-plan preview; deliberately NOT fixed in that commit.** It is the last residue of a variant that five others were split out of *precisely because 500 was wrong* — `PeerUnresponsive` (2026-08-16), `SegmentFailoverExhausted`, `ReplyTruncated`, `ModelIncompleteInSwarm` and `PromptPrivacyUnavailable` (2026-08-11), each moved to 503 with a comment saying 500 "reports a deliberate configuration as a crash" or "tells monitoring the node the user talked to was broken". `PipelineError`'s arm in `classify_error` is the only one of that family with no comment justifying its status, which is the tell: it was never re-examined, only emptied. **Why it was not simply changed**: unlike the five, this variant genuinely carries a MIX — `pipeline/local.rs:559` "Response channel dropped" is arguably ours, while `scheduler/mod.rs:3675`/`:3692` ("Encrypted pipeline requires the requesting node to hold the final shard") are a POLICY REFUSAL, the exact shape `PromptPrivacyUnavailable` was created for and the shard-0 half of the same condition. So the honest fix is not a status edit but the same split again: give the last-shard privacy refusal `PromptPrivacyUnavailable`'s treatment (its own reason, its own hint — note its `#[error]` text names shard 0 and would need generalising, and `error_hint.prompt_privacy_unavailable` is translated into 21 locales), then re-ask what is left. ⚠ **Researched 2026-09-18 and the scope in this entry is WRONG in one place — read this before starting.** The entry says to give the last-shard refusal `PromptPrivacyUnavailable`'s treatment, implying no new user-facing copy. It is not free: that variant's hint (`error_hint.prompt_privacy_unavailable`, `error.rs:860`) reads *"needs the model's **first part** stored here — and it isn't … fetch it with `swarmllm get-model <name>`"*. For the OUTPUT-HEAD case that advice names the wrong end of the model, which is gotcha #295's family exactly — a hint that cannot work, followed confidently. So the split needs either a second hint key or a generalised wording, and either way **21 locales change**, which makes the English wording a decision worth taking before the translations are minted rather than after. The `#[error]` text also hardcodes "shard 0 (the embedding table)" and wants a field naming which end is missing (`PrivacyPiece::{Embedding, OutputHead}` displaying the user-facing phrase) rather than a second variant. **The producers, enumerated 2026-09-18** so the "re-ask what is left" half starts from facts: the two POLICY refusals are `scheduler/mod.rs:3745` and `:3762` (identical text, both the last-shard privacy case). What remains is `"Model has zero layers"` (`scheduler/mod.rs:1867`, and the preview's test at `listing.rs:1319`), `"Pipeline has no segments"` (`pipeline/mod.rs:935`), `"Pipeline completed without producing a result"` (`pipeline/distributed.rs:1622`), `"Response channel dropped"` (`pipeline/local.rs:559`) — all four OURS, i.e. `Internal`-shaped — and the genuine no-route family in `parallax.rs` (7 sites) plus `scheduler/mod.rs:3877`, which is 503-shaped but must NOT become `ModelIncompleteInSwarm` (`assembly_failed_for_lack_of_holders` variant-matches it into a pointless DHT wait). So it is a three-way split, not a two-way one. **Blast radius to check first**: nothing greps for the 500 today, but `failure_log_level` keys off it (500 → `FailureLevel::Error`), so every unassemblable route is currently logged at ERROR — which is also why a node that never hits this variant shows 0 ERROR lines and one that does looks far sicker than it is. `is_transient_remote_failure` and `failure_is_penalty_worthy` key off the VARIANT, not the status, so they are unaffected by a status change alone. Scope: one round, not a fix to bolt onto something else |
| 87 | **An idle node with the dashboard open logs ~100 lines a minute of routing diagnostics for requests nobody made** | ✅ **FIXED 2026-09-18.** `scheduler::Purpose::{Route, Preview}`, a required parameter on `assemble_pipeline_for` threaded into the three inner planners, plus a `route_info!` macro so each line is `info!` for a request and `debug!` for a preview. The diagnostics workflow is untouched — it answers a question about a request the user made, which stays `Route`. ⚠ **This entry's list of eleven line numbers was WRONG: there are twenty**, and one of the nine it missed (`Encrypted pipeline active`) is a line this same entry had measured as one of the six a preview emits — so following the list would have left a sixth of the noise while looking finished. The guard is therefore the ABSENCE of a bare `tracing::info!` in any route-planning function (`every_routing_diagnostic_is_scoped_to_its_purpose`), not a list. **Measured on an isolated node, same 5 previews, before and after**: v0.3.187 released — **15** routing INFO lines; fixed build — **0**; and one real inference request still emits 3 on the fixed build, which is the control that says the workflow survived. Original analysis below. | **MEASURED 2026-09-18.** One `GET /api/admin/models/{id}/pipeline-plan` produces exactly **6 INFO lines** (4 × `DIAG: pipeline candidate`, plus the `Local node has full layer coverage` decision and `Encrypted pipeline active`), measured on the live node by counting the log before and after a single request. The dashboard fires one per VISIBLE model card whenever any peer's hosted-shard total changes (`dashboard.js`, `_pipelinePlanRefreshTimer`, 500 ms debounce), which on a live swarm is every 30-45 s — so ~12 cards × 6 = ~70 lines a burst. On this node that is **61% of node.log**: 13,680 `DIAG: pipeline candidate` + 2,467 `Local node` + 2,463 `Encrypted pipeline active` + 1,544 `DIAG: parallax` + 1,388 `DIAG: assemble_pipeline_for` out of 36,885 lines in 12.5 h, 13.8 MB. **A new user running `swarmllm run` in a terminal sees this scroll past at the DEFAULT level**, describing routes for models they are not using. **Why the cheap fix is wrong**: dropping these to `debug` undoes a deliberate v0.3.152 decision and breaks a documented workflow — `docs/DIAGNOSTICS.md` § "Why did my machine run this itself instead of using the swarm?" answers a real user question from exactly these lines at default level, and `-v` would drown them in all other debug. **The honest fix** is for the scheduler to know it is answering a question rather than routing a request: a `Purpose::{Route, Preview}` parameter on `assemble_pipeline_for`, gating the ~11 `info!` sites in `inference/scheduler/mod.rs` (849, 2242, 2405, 2474, 2498, 2530, 2550, 2563, 2649, 3274, 3479). Real requests keep every line; previews go quiet. A required parameter rather than an `Option`, per `.claude/rules/architecture.md` § "Make the wrong call unrepresentable". Callers today: the preview (`api/admin_models/listing.rs`) and `router/distributed_exec.rs:372`/`:391`. ⚠ **Do NOT infer the purpose from `active_traces`** — `daemon/state/relay.rs::note_predicted_route_cost` already distinguishes them that way and is right to for a no-op, but a real request whose trace failed to register would silently lose its routing diagnostics exactly when they are needed |
| 88 | **The partial-reply salvage does not run on the path distributed requests actually take** | ✅ **FIXED 2026-09-18.** Option (b) from this entry, and the design question it left open is answered: `pipeline::PartialReply` accumulates the reply on the executor as the shared emit helpers produce it, and `keeping_the_partial` is one choke point reading it around all FIVE alternative paths — so nothing records and a sixth path inherits it. The emit helpers were the right place because `streamed_reply_text_goes_through_the_shared_emit_helpers` already obliges a new coordinator in that directory to call them; the obligation existed and was borrowed rather than invented. Recording happens BEFORE `emit_streaming_batch`'s `token_tx`-is-`None` early return, which is the line that made the old code look right. The `prompt_tokens=0 tokens=0` half is fixed too — the accumulator keeps the ids, so `completion_tokens` is honest. Salvaged text is now finalised as well (gotcha #643, found in these same files while researching this). Verified by removing the mechanism and watching the positive test go red with the three null controls staying green. Evidence: `docs/invariants/scheduling.md` § "The salvage was only on the path the fix was written in". Original diagnosis below. | **FOUND 2026-09-18 while reproducing #85; verified by reading all four paths.** `may_salvage` / `note_salvaged_reply` exist because report #028 called a mid-generation failure on a non-streamed request "a total loss": tokens were produced, the request then failed, and the caller got an error with nothing in it. The salvage is implemented **only in `execute_distributed`'s own per-token loop** (`pipeline/distributed.rs:472` and `:508`). But `execute_distributed` first tries three alternative generation paths and calls each with `?` — `try_dsd_distributed`, `try_speculative_distributed` and `try_ngram_only_distributed` — so an error inside any of them leaves `execute_distributed` without ever reaching the loop, the `Err` arm, `may_salvage`, or `note_salvaged_reply`. `grep -c may_salvage` is **0** in `ngram_only_spec.rs`, `dsd.rs` and `speculative.rs`. **And the n-gram path is the DEFAULT one**: `inference.ngram_lookup_enabled` defaults true, it requires only that no draft model is configured (also the default) and that at least one segment is remote, and temperature is deliberately not a condition. Observed directly: a probe request logged `DIAG: try_ngram_only_distributed ELIGIBLE — entering n-gram-only path`, generated ~220 tokens over 40 s, failed, and produced **no** `keeping the partial reply` line and **no** `Pipeline failed and failover (if eligible) was unsuccessful` line — the reply was discarded and the caller got a bare error. The same failure inside the standard loop would have been salvaged. ⚠ **Severity is narrower than it first looks, and this is a hypothesis about frequency, not a measurement**: `forward_through_segments` attempts segment failover internally, and `router` retries a transient failure, so a lost reply needs the failover AND the retry to fail too. What makes it worth a round anyway is WHO is on that path — a node holding nothing runs every request distributed, which is every new user. **Fix shape.** Not "call the helper in three more places": `.claude/rules/architecture.md` § "One invariant, N paths" says a helper nobody is obliged to call will eventually not be called, and this entry IS that rule firing on the salvage itself. The choke point cannot assemble the partial reply either — `execute_distributed` has no access to the tokens each spec path accumulates locally. The two candidates worth costing are (a) make the spec paths return the partial alongside the failure, so the choke point salvages for all of them and a new path cannot opt out, or (b) accumulate the reply in per-request shared state as tokens are decoded, so a salvage is a read rather than something a path must remember to record. (b) also fixes the second thing observed here: `DIAG: request complete` logs `prompt_tokens=0 tokens=0` for these failures although ~220 tokens were generated and streamed, so the trace cannot even say how much was lost. **Reproduction**: the #85 rig above — the window-overflow failure is a convenient deterministic mid-generation failure; any other would do. | Same round as #85: both are about what happens to work already done when a distributed request fails part-way
...
| 55 | A node refuses to load models into graphics memory it is not using | **OPEN, found 2026-09-17, needs a POLICY decision rather than an obvious fix.** The live node logged `VRAM budget full — skipping auto-load … loaded_mb=5124 budget_mb=6160` for six models within 18 SECONDS OF BOOT — before any worker had spawned, with the card holding 2027 MiB total and the pool's own admission logging `committed_mb=1044` in the same second. The 5124 MB is not resident anything: `split_models_committed_mb` sums `estimated_vram_mb` over `split_models`, whose entries are METADATA (`scan.rs`: "Create metadata entry from GGUF header (no GPU loading in main process)"), and `budgeted_memory_predicate` counts an entry whose model has NO live worker because `model_uses_gpu_memory` falls through to `cpu_reason` and answers "would it use the card", not "does it". So the ceiling is consumed by registrations, permanently, in scan order, and never released — a node with several models can locally serve only the first card's-worth it happened to scan, and the scheduler then reports even a 0.5B as "does not fit our GPU" and delegates it to a peer. **This is deliberate as written** — `registering_a_split_model_entry_never_removes_another` and its doc call it a REGISTRATION budget, "may refuse; may never take", and gotcha #402 is why it never evicts — so the question is a policy one: the cap is sized as though every registered model were resident simultaneously, when the pool loads on demand and evicts LRU, so at most a few ever are. Options: count what the POOL actually reserves (`vram_committed_mb()`, one production caller to change), require a live worker in the predicate, or drop the cap and let the pool's real admission be the only gate. **Do not change this silently — it alters what the node offers the swarm.** **Known downstream consumers of the missing registration, enumerated 2026-09-17** so the decision can be made with the blast radius in view: `local_whole_model_segment` (FIXED in item 54) and `SharedState::has_complete_split_model`, which gates `local_fast_path_for` — so on a capped node the API fast path is off for those models too and every request pays a scheduling pass (`schedule_ms=84` on the live node). That one is no longer a correctness loss now that the router path executes as a local generate, but it is the same root. The user-visible symptom to look for is a node delegating work to peers that it could obviously do itself, including models far smaller than its free graphics memory. The downstream damage to local execution was fixed separately in item 54 |
| 56 | A node in Docker on Windows binds its peer-to-peer listener to loopback and cannot be reached | **FIXED 2026-09-17 (`b0f43fb5`), field report #003 against v0.3.182.** `is_wsl2()` reads `/proc/version` for "microsoft"/"wsl", and a container started by Docker Desktop's WSL2 backend inherits the host kernel's string without inheriting the host's networking. The mirrored probe cannot succeed in a container namespace either (no `wslinfo`; the interface layout is the container's own), so EVERY containerised node on Windows took the pessimistic branch: `listen_address=127.0.0.1` plus QUIC/AutoNAT/DCUtR/UPnP/mDNS off. Docker's `-p host:container` mapping forwards to a non-loopback listener INSIDE the container, so the published P2P port went nowhere — the node dialled out fine and looked healthy, while losing exactly the NAT'd peers that needed the disabled features. Reporter's only symptom: "my Docker node sees fewer peers than my Mac on the same account". **This is gotcha #161's shape a second time** — that one narrowed the same predicate with a mirrored-mode exception; it was still too broad. Now one pure truth table, `network::wsl_network_adaptation(is_wsl2, in_container, mirrored)` → `None`/`Mirrored`/`NatSafeDefaults`, with `running_in_container()` OR-ing several signals because none survives every runtime and cgroup version (cgroup v2 in a container can read a bare `0::/`). **The false-positive direction was checked on a real WSL2 box, not assumed**: `/.dockerenv` absent, `/run/.containerenv` absent, `container` unset, `/proc/1/cgroup` = `0::/init.scope`. Pinned by `a_container_on_a_wsl2_kernel_gets_no_wsl_adaptation`, which fails without the exclusion. The documented workaround (set `[network]` keys explicitly) still works and always did, because the override only fills keys the user left unset. ⚠ `health::monitor::maybe_warn_wsl_firewall` gates the same way and already skips containers because the mirrored probe fails there — correct, but by accident; if that probe learns another signal, it needs the container exclusion too |
| 54 | A request routed back to this node never used the prefix cache | **FIXED 2026-09-12, shipped in v0.3.175-alpha**, field-reported on v0.3.174 (report #018): `prefix-cache HIT` appears zero times in a week of logs across several models, on a node holding a complete 14B. A node that would run a model on its PROCESSOR and has peers stands its API fast path aside so the scheduler can consider delegating — deliberate, and `local_fast_path_for` says the cost when nobody better is found is "only a scheduling pass". It was not: the plan came back naming this node, and the pipeline then ran it as a `LayerForward` per token into our own worker, which is the ONE execution path never wired to the prefix cache. Continuous batching, slot admission and n-gram speculation live on the same path and were lost with it. A single-segment plan naming this node is now run as the local generation it is (`try_local_generate_fastpath`), which is what `remote_generate::eligible` has assumed since it was written — it excludes the local node with "local inference is handled by `execute_local`", true only of requests that never reached the router. The span is checked, not assumed: a segment short of either end produces hidden states, not tokens. ✅ **REOPENED AND RE-FIXED 2026-09-17 — the original fix was INERT wherever the model had no `split_models` entry, which on a node holding several models is most of them.** Verified before and after on one isolated node, same data dir and same config, with registration deliberately refused (`inference.max_split_model_memory_mb = 100`, 9-12 refusals logged): the DEPLOYED .186 ran the pipeline and fired the fastpath **0** times, a build carrying the fix ran the pipeline and fired it **1** time, and both answered correctly. `local_whole_model_segment` now falls back to the MANIFEST — the same source `scan.rs` derives `is_first`/`is_last` from — when no registration exists, and requires this node to hold shard 0 and the last shard so the worker cannot be handed a whole-model range with no embedding table (gotcha #187's crash). Pinned by `an_unregistered_whole_model_is_still_a_local_generate` (fails without the fallback) and `the_fallback_declines_when_an_end_shard_is_missing`. ⚠ **The CAUSE of the missing registrations is still open and is its own item** — see the registration-budget finding below. What follows is the original diagnosis. `try_local_generate_fastpath` fired **0 times** in a full node log (the DIAG is `info!`, the node runs at `info`, so the source is complete) while the scheduler produced single-local-segment plans and three pipelines executed. Declining gate established by elimination — not TP, LoRA or images (`fastpath_request_disqualified` false), `segments==1`, the segment names this node, range `(0,28)` — leaving `get_split_model_meta`, which answers `None` unless `split_models` holds an `is_complete` entry. **`split_models` has exactly ONE production writer, `auto_manage/scan.rs:744`**, so the fastpath is available only for a model auto-manage chose to load. For the observed request the scan declined in the same pass — `VRAM budget full — skipping auto-load … estimated_mb=2214 loaded_mb=5124` — and the process pool then admitted that identical model seconds later with `headroom_mb=1977`. Two loaders, two budgets, disagreeing; the scan's refusal is what keeps the fastpath permanently off, so the request ran as `LayerForward` per token and lost the prefix cache, continuous batching, slot admission and n-gram speculation — exactly the loss #018 reported. Corroborating: `cached_mb=0` in every KV-admission DIAG on this node, and `prefix_cache.lookup` exists only in `handle_generate` and the batched pool path, never in `handle_forward`. **Shape of the fix, not yet built**: the fastpath consumes only `meta.layer_range` from that call — not the template, bos or eos — i.e. it is gated on the model being LOADED to learn a number that is available without loading. Answer 'does this node hold the complete model' from the geometry (`SharedState::gguf_meta_for`, which learns from the local `gguf_header.bin`) or the shard manifest, and keep the span check exactly as strict, since 'a segment short of either end produces hidden states, not tokens' is the property that matters. It touches the token hot path, where a mistake is a silently wrong reply, so it wants its own session. **Do not widen it into the two-budget disagreement** — that is a separate finding and is logged beside this one |
| 55 | Stop belonged to the page, not to the chat you were looking at | **FIXED 2026-09-12, shipped in v0.3.175-alpha**, reported live on v0.3.174 (report #015). `send-btn`/`stop-btn` exist once in the document and every session shares them, so opening a second chat while the first was answering showed Stop over an empty compose box — and the single page-level `AbortController` meant pressing it cancelled the FIRST chat's reply. Sending was blocked there too, by the same flag. Both now hang off `S.streaming[sessionId]`. Fixing it exposed a second gap worth having: switching away from a streaming chat left the reply writing into detached DOM, so it appeared to stop; the record now carries the live nodes and `renderMessages` hands them back |
| 56 | The memory bar read 13 MB for a worker holding 13 GB, on macOS | **FIXED 2026-09-12, shipped in v0.3.175-alpha**, field-reported on v0.3.174 (report #017) from a 16 GB Mac mini. macOS keeps two accountings of a process's memory: `sysinfo` reads `pti_resident_size` from `proc_pidinfo`, Activity Monitor shows `ri_phys_footprint` from `proc_pid_rusage`, and they can differ by orders of magnitude. Both are now read and the larger reported, because under-reporting is the failure that matters — a bar near zero on a nearly-full machine says the node is idle when it is the thing filling the memory. ⚠ **The reporter's stated mechanism (mmap'd pages invisible to the process figure) is probably backwards** — footprint is the accounting that EXCLUDES clean file-backed pages, and footprint is what showed 13 GB — so the fix deliberately does not depend on which one is right. Untestable from Linux; CI's macos-15 job compiles and runs it, and the reporter can confirm the figure |
| 57 | `max_bandwidth_mbps` looked like a cap on everything and is not | **FIXED 2026-09-12, shipped in v0.3.175-alpha** (suggestion, 2026-09-11). It IS enforced — `shard_upload_mbps` throttles serving model pieces in `network::manager::requests` — but nothing else, so a user who set it to 1 Mbps, measured ~11 Mbps and concluded it was a no-op was half right in the way that matters. The setting now says what it covers, in the settings panel and in `config/default.toml`. The larger half was that **there was no way to ask a node how much traffic it was generating**: the reporter had to stop the daemon and diff `/sys/class/net`. libp2p's transport counters are now on (`with_bandwidth_metrics`, the only builder phase where a transport can be wrapped, so it sees every protocol) and surfaced in `swarmllm status`, the dashboard and `/metrics`. Absent rather than zero when nothing is counting — a confident 0.0 Mbps is the reading this replaces. ⚠ **The 11 Mbps itself is NOT reproduced**: a fresh node with 5 peers measured ~5 kbps here and a 12-hour-old node with many models ~0.8 Mbps, so the reporter's figure needs their own node's number to attribute |
| 47 | `tojson` escaped HTML into every tool schema, and rejected the keywords templates pass it | **FIXED 2026-09-11**, found by research before running a model rather than by a field report. minijinja's builtin `tojson` rewrites `<`, `>`, `&` and `'` as `\u003c` and friends — it is written for embedding JSON in a web page — so a tool described as "the user's location" reached the model as `the user\u0027s location`, on **every tool-carrying request to every model whose template renders tools through the filter** (Qwen3, Llama-3.1, GLM-4, Hermes…). `transformers` overrides the same filter for exactly this reason and says so in a comment; llama.cpp's minja does not escape either. The builtin also takes `indent` alone and calls `Kwargs::assert_all_used`, and an error inside a filter fails the WHOLE render — so GLM-4's `tojson(indent=4, ensure_ascii=False)` meant every GLM-4 request carrying tools was answered by a fallback prompt with none of the model's own `# 可用工具` framing. `chat_template::tojson` now implements the signature `transformers` defines (`ensure_ascii`, `indent`, `separators`, `sort_keys`), with Python's separator defaults, so a bare `\| tojson` is byte-identical to HF's. ⚠ minja rejects `ensure_ascii` too — **the reference had the bug, which is a reason to read the reference and then think, not to copy it.** Residual key ordering is item 46 |
| 45 | Streamed replies stopped at exactly 65 tokens and called it a natural stop | **FIXED 2026-09-11**, field-reported on v0.3.172: on two models, two devices, with and without tools, purely from setting `"stream": true`, while the same request non-streaming ran to 1000 tokens. **Two defects in series.** The in-process llama.cpp executor (`-m`, so only a GPU build — `cuda` and `windows-gpu` are the feature sets that pull in `llama`; a CPU build refuses `-m` outright, which is why nothing reproduced here at first) generates on the calling thread and never yields, and it was called straight from a router task — so the Tokio worker running it could not poll the task draining tokens into the response, and **nothing streamed at all**: every delta arrived when generation finished. The 64-slot channel then filled, and `try_send(..).is_ok()` as the keep-generating signal read a FULL channel as a departed client and ended the reply at 65 — reported `"stop"` because `GenerationResult` derives `finish_reason` from `completion_tokens >= max_tokens` alone. Now `executor::without_starving_the_runtime` (all seven in-process `generate*` calls) and `api::sse_send_live_blocking` (the blocking sibling of `sse_send_live`, terminal events included). Measured on a 0.5B: 65 tokens in 121 s → 390 deltas ~20 ms apart, matching the non-streaming baseline. ⚠ Fixing only the backpressure half made it WORSE — same 65 tokens, now after 121 s of waiting out two stall limits — and that is how the starvation was found. ⚠ Two null controls had to be repaired before either test could fail. Evidence: `docs/invariants/api-surfaces.md`; gotchas #555, #556 |
| 44 | A model whose template refuses a system role could not be told about its tools | **FIXED 2026-09-11**, found by `examples/family_conformance.sh` in the same run that verified 43. Gemma-2's own template does not ignore a system turn, it `raise_exception`s on one — and `describe_tools_in_prose` delivers the tool description AS a system message, so **every Gemma-2 request carrying tools rendered through `gemma_fallback` rather than the template that shipped with the model**, and so did any request carrying a caller's own system message. The remedy both publishers document is to prepend the system text to the first user turn; `fold_system_into_first_user` now does that as a RETRY on the failure path, so the template has already answered for itself and no model that renders a system turn today can have its prompt changed. Deliberately not keyed on `template_expects_system`, which answers false for any template containing `raise_exception` — right for deciding whether to ADD a default prompt, wrong as grounds for rewriting a prompt that already renders. Evidence: `docs/invariants/api-surfaces.md`; gotcha #554 |
| 43 | Phi-4-mini — and every other partial-RoPE model — could not serve a single request | **FIXED 2026-09-11.** Found by `examples/family_conformance.sh` on its first run covering that model: every request died before a token with `attn: slice-set only supports contiguous tensors`. **The cause first recorded here was wrong.** It read GQA + fused QKV, from the two models' most visible difference; `FusedSlice::forward` already ends in `.contiguous()`, so the split was innocent. The real discriminator is **partial RoPE** — `rope_dim < head_dim`. `apply_rotary_emb` left the pass-through half as a `narrow` view, and candle's `Tensor::cat` answers with a TRANSPOSED VIEW rather than a fresh buffer when any argument is non-contiguous and `dim != 0`; `slice_set` refuses one, and that is how the KV cache writes K. Phi-3.5 serves fine because `head_dim == rope_dim == 96`; Phi-4-mini is 128/96 because `head_dim = embedding_length / head_count` and it has 24 heads to Phi-3.5's 32 — so GQA correlates perfectly across those two models and explains nothing. **Two copies of the branch existed and both were wrong** (`LayerWeights`, `Qwen35AttnWeights`), so GLM-4 and Qwen 3.5 were equally dead and nobody had run one. Now one `layers::rope_over_heads`, with `SeqCache::append` contiguous-ifying its source so a new producer cannot bring the class back. ⚠ `test_partial_rope_glm4_style` covered this exact feature and could not see it — it asserts the output shape and the pass-through values, both true of the broken view, and never reached a cache write. Evidence: `docs/invariants/inference.md`; gotchas #552 (the wrong cause, corrected in place) and #553 |
| 38 | A model's declared EOS was not the token it ends its turn with, so Phi replies never stopped | **FIXED 2026-09-11.** Two field reports on v0.3.171 titled "tool calling broken" (Qwen3-8B, then Phi-4-mini picked deliberately as a control); most of what they described had nothing to do with tools. Phi-3/3.5/4 GGUFs declare `eos_token_id = <|endoftext|>` and close every turn with `<|end|>` — a different token, 32007 in Phi-3.5 — with no `eot_token_id` key to say so, and `eos_tokens_with_arch_fallback`'s per-family extras sat behind `if ids.is_empty()` so a model that declared *anything* never reached them. Reproduced **with tools removed**, which is what proved the framing wrong: "Say exactly: hello", `max_tokens: 120` → `finish_reason: "length"`, 120/120 tokens, a fabricated second assistant turn then a fabricated Chinese user turn. After: 9 tokens, `finish_reason: "stop"`. `end_of_generation_ids_from_vocab` now searches the vocabulary BY NAME, llama.cpp's approach, and is merged in unconditionally at all four EOS-resolving paths. **The harmony exclusion is load-bearing**: for gpt-oss / solar-open `<|end|>` separates messages inside one reply, so the obvious one-line fix would have truncated every such reply at its first message. `</s>` and `<eos>` are deliberately NOT adopted though upstream lists them. Evidence: `docs/invariants/inference.md`; gotchas #539-#541 |
| 39 | One cause, two symptoms, because the decoder differs by vocabulary family | **Same fix as 38, recorded separately because it nearly cost a second investigation.** On a SentencePiece vocab `decode_token_impl` maps any `<…>` token to empty, so `<|end|>` vanished and the run-on was invisible; on a GPT-2 byte BPE vocab the same token decodes to its own characters, so it LEAKED into visible content. The reporter saw the second on Phi-4-mini and reproduction here produced the first on Phi-3.5 — same family, same cause, and for a while they read as unrelated bugs. It also decides the fix: on the SPM path the text never contains `<|end|>`, so a stop STRING can never match it and only the token id works on both. Gotcha #540 |
| 40 | A model built for function calling had its native output format simply unparsed | **FIXED 2026-09-11.** Phi-4-mini-instruct emits `functools[{"name", "arguments"}, …]` or that array wrapped in `<|tool_calls|>…<|/tool_calls|>`; `parse_tool_calls` returned `None` for both, so every call it made came back as raw assistant text with no `tool_calls` field — no error, `finish_reason: "stop"`, an agentic client executing nothing. The reporter had chosen this model *as a control*, reasoning a purpose-built model should be the case that works; the control was the finding. `try_phi` added before `try_llama3` (which takes a bare object and would swallow a more specific family's payload), and its marker added to `CALL_MARKERS` — `functools` sits BEFORE the brace, so without that the streaming surfaces emitted the marker itself as content. Gotcha #542 |
| 41 | A template that mentions `tools` without reading the variable left the model told nothing | **FIXED 2026-09-11.** Phi-4-mini's tool branch tests `'tools' in message and message['tools'] is not none` — a per-MESSAGE field, not the top-level variable it is handed. So `template_renders_tools`' substring check passed, the prose description was suppressed as redundant, the branch never fired, and the model learned **nothing at all** about its tools. That is worse than either branch alone; it is the gap between them, and it is exactly the reported "no engagement with the `tools` array whatsoever, despite one being present and directly relevant". `render_shows_the_tools` is now a post-condition on the render — the same shape as `render_kept_the_last_question` — and a render that was handed tools and shows none of their names is discarded for the prose path. General, so it catches any such template rather than just this one |
| 42 | A fenced tool call followed by an explanation was not parsed | **FIXED 2026-09-11.** `strip_code_fences` only stripped when the WHOLE reply was one fenced block, so the common chatty shape — a fenced call plus "Cette demande JSON spécifie…" after the closing fence — let the trailing fence and prose reach the brace scan. Measured on phi-3.5-mini: the identical call parsed alone and parsed fenced, and returned `tool_calls: null` with the JSON visible to the user once prose followed. The block is now taken up to its closing fence, whatever follows. A **bare** `{"name", "arguments"}` object inside prose still does not parse and must not — `try_llama3` requires the whole text to be that object, and loosening it would turn any reply discussing a JSON value with a `name` field into a call the user never approved |
...
| 35 | A model's own tool framing was never rendered; every model got a hand-written JSON instruction | **FIXED 2026-09-10.** Field-reported against v0.3.170: Qwen3-8B with `tools` present emitted repeated malformed special tokens and made no call, through two independent clients. `apply_chat_template` had no `tools` parameter, so `{%- if tools %}` was false on every request ever made and Qwen3's own `# Tools` / `<tool_call>` framing was unreachable. A second defect was hiding under it — `minijinja` was built without the `json` feature, so `tojson`, which every real tool-rendering template calls, was an unknown filter that failed the WHOLE render. Verified end-to-end 3/3 on Qwen3-1.7B: `finish_reason: tool_calls` with correct arguments. ⚠ **The reporter's `<|start|>` garbage is NOT reproduced here** and may be a separate defect — see the row below. Evidence: `docs/invariants/api-surfaces.md` |
| 34 | `release_escrow` settled a balance against a write that had failed | **FIXED 2026-09-10, same day it was reported.** Found by a contributor who modelled `credit/escrow.rs` in TLA+ and ran TLC against it (issue #21) — a persist failure was logged and the balance reconciled anyway, so `EscrowManager::new` re-loaded the still-`Pending` entry at the next restart and `cleanup_expired` refunded the full reservation on top of the settlement. Replayed here: **500 in, 40 of compute consumed, 560 out**. Dormant credits mean nothing was mis-served, but the accounting could inflate. The other two paths in the same file already reverted correctly, one of them with a comment explaining exactly this hazard. Evidence: `docs/invariants/state-and-config.md` § "A settle that cannot be written down has not happened" |
...
shards are on an HDD rather than the Mac's SSD). Verified: nothing the scheduler reads is
derived from a disk benchmark. Decode cost for a processor-only local node IS measured
(`observed_latency_ms_per_layer`), but it is sampled from an already-loaded memory-mapped
model, so it reflects RAM bandwidth; prefill cost for the local node is unconditionally
`None` and priced from the capability prior; and `mem_bandwidth::measured_gbps` streams a
256 MB in-memory buffer with no file I/O at all. So an HDD-backed node is priced exactly
like an SSD-backed one with the same RAM and CPU — **no penalty and no credit.** The one
place it bites is real and unpriced: the first, cold load of a model. That cost is a
one-off per load, `COLD_MODEL_LOAD_ALLOWANCE_SECS` already widens the segment TIMEOUT for
it (and only the timeout — it plays no part in the priced search), and a node thrashing
its disk during decode does eventually show up in the measured decode figure. Recorded
rather than fixed: pricing a cold load would need a disk measurement the scheduler has
never taken, to change a decision it makes once per model.
...
## The UI says both "shard" and "part" for the same thing, everywhere (open, 2026-09-13)

Carried over from the 2026-09-12 new-user sweep, which counted the jargon and
filed it under `activity.*` — "the console feed, which matters less now".
**Counted again against `en.json` on 2026-09-13 and that framing is wrong: the
split is UI-wide, not feed-local.**

| term | strings outside `activity.*` | inside |
|---|---|---|
| `shard` | 46 | 18 |
| `part`/`parts` | 53 | 7 |
| `peer` | 66 | 8 |

So the two vocabularies are already interleaved across the whole interface, and
in places within one panel: `activity.shard_p2p_exhausted` says "part
{shard} of {model}" while `activity.shard_download_started` two lines above it
in the same feed says "Downloading shard {shard}". The .178 model list settled
on **part** (`shard.row.part_n_of_m` = "Part {n} of {total}") and the .177 copy
on **computer** for a peer, and neither migration was finished.

**Why this is not a quick pass.** Rewording `activity.*` alone — the sweep's
suggestion — leaves 46 "shard" strings and 66 "peer" strings elsewhere, so it
converts a split feed into a split page: a "Prune History" panel beside an
activity line reading "Removed part 3 to free space". Half a rename reads worse
than none, because the reader now has to work out whether the two words mean
different things. Roughly 112 strings across 21 locales, and the decision
about each one is whether the technical word is load-bearing there.

Some of it must stay technical and should be listed before anyone starts:
`activity.invite_lan_only`, `activity.upnp_mapped` and
`activity.inbound_blocked` name UPnP, the P2P port and a relay node because the
user has to act on them, and the sweep flagged that style as already good.

**Precedent for doing it properly**: R126 did exactly this for the 19 `enc.*`
keys plus `models.encrypted_pipeline` — "first piece of the model" / "last piece
of the model" replaced the shard vocabulary across all 21 locales in one pass,
and drew the distinction between "end-to-end encrypted" and "encrypted in
transit" while it was there. One term, every surface, one commit.
...
### The honest half shipped 2026-09-13

`networkMap.renderRouteKey` is written by `renderRecentRoutes` — the same
function that draws the arcs, on every call including the empty one, so the key
cannot fall out of step with the map. On a node that has run nothing across
regions it says so and names what will make lines appear.

It also closed a gap that had nothing to do with the empty state: **the arcs
were painted in two colours and were in no key at all.** A key must list every
value the thing it explains can paint (`docs/invariants/frontend.md`), and this
one listed node density, the map's other visual channel, and stopped there. The
swatch reuses `.map-route-arc` itself rather than restating its colours, for the
same reason the shard legend's swatches are `.avail-seg`. The "failed" entry
appears only when a failed arc is actually on the map — a key listing a colour
nothing is painted in reads as a warning about something that has not happened.

**Seeding the map from the swarm's own traffic is still open**, and still needs
the new gossip message and the privacy decision above.

## The per-model activity ticker lived inside the expanded model card (FIXED, 2026-09-13)
...
feature nobody had asked to drop. It was still a log inside a card.

**It was also a second rendering of data the Activity panel already had.**
`_pushEntry` runs before the ticker was ever fed, so every event reached both —
the ticker duplicated them into its own store (`_modelEvents` /
`_modelNetEvents`, capped at 15 each), with its own signature cache, its own
expand/collapse handler and its own "ago" refresh loop walking
`.model-ticker-time` every 30 s. About 2.7 KB of JS and 60 lines of CSS for a
copy.

`App.notifications.filterByModel(id)` now narrows the Activity **and** Network
panels to one model, and the card carries a link to it. Three things that had
to be right:

- **The filter is part of the render signature.** `_renderEventLog` skips the
  rebuild when `topTs + entries.length` is unchanged; setting a filter changes
  neither, so without adding it to the signature the panel would have kept
  showing every model. This is the same shape as gotcha #585 — a cache keyed on
  less than the thing it caches.
- **Filter first, then count.** The header count is computed from the same list
  the rows come from, or a filtered log reads "42 events" over four rows.
- **A filtered log with no matches is not an empty log.** "No activity yet" over
  a model that simply has none reads as though the whole node is idle;
  `activity.none_for_model` says which it is.

`activity.count` also gained `_one`/`_other` forms across all 21 locales. It had
always been able to print "1 events", and filtering to one model is what makes
that the common case rather than a rare one.

## A disputed shard is kept but the disagreement is never settled (open, 2026-09-13)

Fallout from the fix recorded in the triage table as item 60. A shard we hold
whose bytes disagree with a hash that has **no origin backing** is now kept
rather than quarantined — which stops the data loss — but nothing then resolves
which side is right. The node keeps serving bytes it believes in, re-reports the
disagreement on every verification pass, and stays that way indefinitely.

**The obvious move does not work, and this is the trap.** `mark_shard_for_repair`
looks like the answer and is a no-op here: `complete_pending_shard_fetches`
begins its loop by treating any shard whose file is **on disk** as already
repaired and clearing the mark. The quarantine path only ever worked because it
had deleted the file first. The first cut of the fix called it anyway and the
commit message claimed a settlement that could not happen; corrected the same
day by reading the consumer instead of the producer.

Three candidate designs, cheapest first:

1. **Self-attestation.** When background verification SUCCEEDS, record that our
   own bytes attest that hash — the hash was just computed, so this is free —
   and let `register_manifest` refuse a contradicting unbacked claim for a
   self-attested shard, at lower precedence than `origin_verified`. Then the
   dispute never forms and nothing is downloaded at all. ⚠ **The objection to
   think through first**: a coherent different build is exactly a file and a
   manifest that agree, so self-attestation would make a node permanently refuse
   to converge on the swarm's dominant build. That may be fine — we hold a valid
   build, and `shard_holders` already filters by build tag so peers on another
   build do not route to us — but it is a policy choice, not an obvious win.
2. **Settle from the origin without deleting first.** A distinct marker (not
   `shards_needing_repair`, whose drain is what breaks here) that fetches the
   origin's copy over the top while we keep serving ours. Converges in one
   fetch and records provenance, which immunises the shard permanently — this
   is what happened by accident on 2026-09-13, minus the outage. ⚠ Costs a full
