...
stop_by_port() {
    for p in $(pgrep -x swarmllm 2>/dev/null); do
        if tr '\0' ' ' 2>/dev/null < "/proc/$p/cmdline" | grep -qE -- "(--port|-p) $1( |$)"; then
...
echo "Stop with: for i in \$(seq 0 $HOLDERS); do for p in \$(pgrep -x swarmllm); do tr '\\0' ' ' 2>/dev/null < /proc/\$p/cmdline | grep -q -- \"-p \$((${BASE_PORT}+i))\" && kill \$p; done; done"
